...
class genrateInvoiceController extends Controller
{
    ...
    public function getTotalAmount(Request $request)
    {
        ...
        if ($cata == "school fees") {
            $records = FeesMap::where('status', '=', 1)->where('invoice_generated', '=', 0)->where('standard', '=',  $request->input('std'))->where('group', '=', $request->input('group'))->where('fees_heading', '=', $request->input('cat'))->orderBy('id', 'DESC')->get();
        } else {
        ...
        if ($cata == "school fees") {
            ...
            $data[] = ['heading' => 'school fees', 'total' => $records->sum('amount'), 'glance' => $concatenatedHeadings];
        } else {
            $data[] = ['heading' => 'others', 'total' => $records->sum('amount'), 'glance' => $concatenatedHeadings];
        }
    ...
    public function discountTotalAmount(Request $request)
    {
        ...
        if ($user) {
            return response()->json(['message' => 'validator sucess Data inserted', 'data' => $data], 200);
        }
    }
    // public function readDiscount(Request $request)
    // {

    //     $users = SchoolFeeDiscount::all(); // retrieve all users from the 'users' table
    ...
    //         $users[$key]['student_id'] = $user ? $user->name : '';
    //         $users[$key]['roll_no'] = $user ? $user->roll_no : '';
    //         $users[$key]['standard'] = $user ? $user->standard : '';
    //         $users[$key]['sec'] = $user ? $user->sec : '';
    //         $users[$key]['discount_cat'] =  $users[$key]['invoicefeescat'] . "-" . $users[$key]['discount_cat'];
    //         //  $users[$key]['status'] = $user ? $user->status : '';
    //         if ($users[$key]['status'] == '0') {
    //             $users[$key]['status'] = 'Generated';
    //         } else {
    //             $users[$key]['status'] = 'Not Generated';
    //         }
    //     }
...
$fromDate = !empty($fromDate) ? Carbon::parse($fromDate)->startOfDay()->toDateTimeString() : null;
$toDate = !empty($toDate) ? Carbon::parse($toDate)->endOfDay()->toDateTimeString() : null;


$users = SchoolFeeDiscount::when($fromDate, function ($query) use ($fromDate) {
...
// return $users;
    ...
    $filteredUsers = $users->filter(function ($student) use ($grade) {
        $query = User::select('id', 'name', 'roll_no', 'standard', 'sec')
        ...
        if ($user) {
            ...
            $student->discount_cat = $student->invoicefeescat . "-" . $student->discount_cat;
            $student->status = $student->status == '0' ? 'Generated' : 'Not Generated';
$student->created_att =  Carbon::parse($student->created_at )->format('d/m/Y h:i A');
    ...
    public function genrateForGrade(Request $request)
    {
        ...
        $userDatas = User::where('status', '=', 1)->where('standard', '=',  $request->input('std'))->where('user_type', '=', 'student')
    ->when($request->has('stdid'), function ($query) use ($request) {
        ...
        foreach ($userDatas as $userData) {
            ...
            $concatenatedHeadings = '';
            $fees_items_details = array(); // Fees items details json for invoice print
            ...
            if ($cata == "school fees") {
                ...
                if ($records) {
                    ...
                    foreach ($records as $record) {
                        ...
                        // $concatenatedHeadings .=  $record->fees_heading . '-' . $record->fees_sub_heading . '-' . $record->amount . '<br>';
                        $concatenatedHeadings .=  $record->fee_heading . '[' . $record->fee_sub_heading . ']:  Rs.' . $record->amount . '<br>';

                        $fees_items = (object) [];
                        $fees_items->fees_heading = $record->fee_heading;
                        $fees_items->fees_sub_heading = $record->fee_sub_heading;
                ...
                $records = StudentFeesMap::where('status', '=', 1)->where('invoice_generated', '=', 0)->where('student_id', '=', $userData->id)->where('standard', '=',  $request->input('std'))->where('fee_heading', '!=', 'School Fees')->orderBy('slno', 'DESC')->get();
                if ($records) {
                    foreach ($records as $record) {
                        // $concatenatedHeadings .= 'Fee Heading: ' . $record->fees_heading . ' - Subheading: ' . $record->fees_sub_heading . ' - Amount: ' . $record->amount . '<br>';
                        $concatenatedHeadings .=  $record->fee_heading . '[' . $record->fee_sub_heading . ']:  Rs.' . $record->amount . '<br>';
                ...
                if ($invoice_lists) {
                    foreach ($invoice_lists as $invoice_list) {
                        if ($invoice_list->invoice_no) {
                            array_push($usedInvoiceIdsNos, strtoupper($invoice_list->invoice_no));
                        }
                    }
                }
                $currentYear = date('y');
...
$dues1 = $generateInvoiceViewModel->getMostRecentDues($userData->id,$data->fees_cat);
$excess1 = $generateInvoiceViewModel->getMostRecentExcess($userData->id,$data->fees_cat);
                ...
                $dataFeeMaps = [
                   ...
                   // 'invoice_no' => strtoupper(randomId(1111, 9999, 'SVS' . date('dMy'), $usedInvoiceIdsNos)),
                    ...
                    'fees_items_details' => json_encode($data->fees_items_details),
                    ...
                    'amount' =>  $amount,
                    'previous_pending_amount' => $pending_amount ? $pending_amount : 0,
                ...
                $GenerateInvoiceView = GenerateInvoiceView::create($dataFeeMaps);
                if (!$GenerateInvoiceView || !$GenerateInvoiceView->slno) {
    // Log or handle the error
    throw new Exception('Invoice creation failed or returned null ID.');
}
 ...
 DB::table('generate_invoice_views')
    ...
    ->update([
        ...
        'updated_at' => now() // Update the timestamp
                ...
                if ($GenerateInvoiceView) {
                    ...
                    foreach ($records as $record) {
                        // $concatenatedHeadings .= 'Fee Heading: ' . $record->fees_heading . ' - Subheading: ' . $record->fees_sub_heading . ' - Amount: ' . $record->amount . '<br>';
                        $concatenatedHeadingsresult .=  $record->fee_heading . '-' . $record->fee_sub_heading . '-' . $record->amount . '<br>';
                    ...
                    if ($cata == "school fees") {
                        $recordFs = FeesMap::where('status', '=', 1)->where('invoice_generated', '=', 0)->where('standard', '=',  $request->input('std'))->where('fees_heading', '=', $request->input('cat'))->orderBy('id', 'DESC')->get();
                        ...
                        $recordFs = FeesMap::where('status', '=', 1)->where('invoice_generated', '=', 0)->where('standard', '=',  $request->input('std'))->where('fees_heading', '!=', 'school fees')->orderBy('id', 'DESC')->get();
                        foreach ($recordFs as $recordF) {
                            // $concatenatedHeadings .= 'Fee Heading: ' . $record->fees_heading . ' - Subheading: ' . $record->fees_sub_heading . ' - Amount: ' . $record->amount . '<br>';
                            $recordF->invoice_generated = 1;
                            $recordF->save();
    ...
    public function cashgenrate(Request $request)
    {
        ...
        $spexcess_amount = null;
                $invoiceDetails = GenerateInvoiceView::find($id);
        if (!$sponsor) {
            $excess_amount = User::where('id', $invoiceDetails->student_id)->value('excess_amount') ?? '';
            ...
            if ($spexcess_amount->excess_amount < $amount) {
                return response()->json(['message' => 'Sponsor has less amount than the entered amount'], 422);
            }
        }
       if ($amount === null || $amount == 0) {
            return response()->json(['message' => 'Invalid amount. Please provide a valid amount.'], 422);
        }
        if ($invoiceDetails) {
            $totalInvoiceAmount = $invoiceDetails->total_invoice_amount;

            if ($invoiceDetails->invoice_pending_amount === null || $invoiceDetails->invoice_pending_amount == 0 || $invoiceDetails->invoice_pending_amount == 0.00) {
                if ($spexcess_amount!== null) {
                    if ($totalInvoiceAmount < $amount) {
                        return response()->json(['message' => 'Add Sponsor Excess amount first to the sponsor totalInvoiceAmount'.$totalInvoiceAmount.'$amount'.$amount], 422);
                    }
                }
                ...
                if ($totalInvoiceAmountActual > $amountActual) {
                    ...
                    $upexcess_amount = number_format($balance_amount, 2, '.', '');
                    $pending_amount = 0.00;
                    $payment_status = "Paid";
                } else {
                    $pending_amount = 0.00;
                ...
                $dataInvoicePaymentMaps = [
                    ...
                    'transaction_amount' => $invoiceDetails->total_invoice_amount,
                ...
                if ($excess_amount !== null) {
                    $user = User::where('id', $invoiceDetails->student_id)->first();
                    if ($user) {
                ...
                if ($spexcess_amount!== null) {
                    // if ($invoiceDetails->invoice_pending_amount > $amount) {
                        if ($invoiceDetails->invoice_pending_amount < $amount) {
                        return response()->json(['message' => 'Add Sponsor Excess amount fist to the sponsor'.$invoiceDetails->invoice_pending_amount . $amount], 422);
                    }
                }
                $totalInvoiceAmountActual =  number_format($invoiceDetails->invoice_pending_amount, 2, '.', '');
                $amountActual = number_format($amount, 2, '.', '');
                if ($totalInvoiceAmountActual > $amountActual) {
                    $balance_amount = $totalInvoiceAmountActual - $amountActual;
                    $pending_amount = number_format($balance_amount, 2, '.', '');
...
public function invoiceSearch(Request $request)
{
    ...
    $options = $query->orderBy('slno', 'desc')->get(['slno', 'invoice_no', 'student_id', 'roll_no', 'name', 'sponser_id']);
...
// Log the input values
        ...
        if ($searchTerm) {
            $studentsQuery->where(function ($query) use ($searchTerm) {
                ...
                $columns = [
                    'slno', 'invoice_no', 'student_id', 'roll_no', 'name', 'sec', 'hostelOrDay',
                    'sponser_id', 'email', 'standard', 'twe_group', 'actual_amount', 'amount',
                    'previous_pending_amount', 'total_invoice_amount', 'discount_percent', 'fees_glance',
                    'fees_cat', 'fees_items_details', 'discount_items_details', 'date', 'acad_year',
...
$totalPaidAmount = ByPayInformation::whereIn('invoice_id', $studentst->pluck('slno'))
...
$totalInvoicePendingAmount = ByPayInformation::whereIn('invoice_id', $studentst->pluck('slno'))
    ...
    ->value('due_amount');
      ...
      //  $count = GenerateInvoiceView::count();
        ...
        foreach ($students as $key => $student) {
            ...
            value="' . $student['slno'] . '"
            onclick="handleCheckboxClick(\'' . $student['slno'] . '\')"
        />';


            $student['sponseruser_id'] = $student['sponser_id'];
            $student['sponser_name'] =  optional(User::where('id', $student['sponser_id'])->first())->name;
            $student['student_excess'] = optional(User::where('id', $student['student_id'])->first())->excess_amount;

            $student['created_by'] = optional(User::where('id', $student['created_by'])->first())->name;

            $student['sponser_id'] = optional(User::where('id', $student['sponser_id'])->first())->name;
            ...
            if ($student['payment_status'] != "Invoice generated") {
                ...
                foreach ($invoiceLists as $invoiceList) {
                    $paymentTransactionId = $invoiceList->payment_transaction_id;
        ...
        return response()->json(['data' => $students  ,'draw' => $draw, 'recordsFiltered' => $count, 'recordsTotal' => $count,'totals' => [
    ...
    //     $students = GenerateInvoiceView::orderBy('slno', 'desc')->get();
    //     // $count = GenerateInvoiceView::where('standard', $standard)->count();
    ...
    //     //     $students[$key]['created_by'] = $user ? $user->name : '';
    //     // }
    //     foreach ($students as $key => $student) {
    //         $student['sponseruser_id'] =  $student['sponser_id'];
    ...
    //             $invoiceList = DB::table('invoice_lists')
    //                 ->where('invoice_id', '=', $invoice_id)
    //                 ->where('status', '=', 'success')
    //                 ->first();
    ...
    //                 $students[$key]['url'] = $url;
    //             } else {
    //                 // Handle the case where invoiceList is not found or status is not 'success'
    //                 $students[$key]['url'] = '';
    //             }
    //         } else {
    //             $students[$key]['url'] = ''; // Set URL to an empty string
    //         }
    //     }
    ...
    public function listgenrateById(Request $request)
    {
        ...
        if ($students) {
            ...
            // return response()->json(['data' => $students]);
            $paymentInformation = DB::table('by_pay_informations')->where('student_id', $students->student_id)->where('type', $students->fees_cat)->latest('id')->first();
            $students->excessAmount = $paymentInformation->s_excess_amount ?? '0';
            $students->hexcessAmount = $paymentInformation->h_excess_amount ?? '0';
    ...
    //         $query->where('acad_year', $acad_year)->whereBetween(DB::raw('DATE(created_at)'), [$from, $to])
    //             ->orderByDesc('slno');
    //     }
    ...
    //         //  onchange="handleCheckboxChange(\'' . $student['slno'] . '\')"
            ...
            if (!empty($fees_items_details)) {
                ...
                $dataFeeMaps = [
                   ...
                   // 'invoice_no' => strtoupper(randomId(1111, 9999, 'SVS' . date('dMy'), $usedInvoiceIdsNos)),
                    ...
                    'sec' => $userData->sec,
                    'hostelOrDay' => $userData->hostelOrDay,
                     'sponser_name' => optional(User::where('id', $userData->sponser_id)->first())->name,
                    'email' => $userData->email,
                    'fees_glance' =>  $data->fees_glance,
                    'fees_cat' => $data->fees_cat,
...
implode(', ', array_map(function($item) {
    ...
    return implode(', ', array_map(fn($key, $value) => "$key: $value", array_keys($itemArray), $itemArray));
}, $dis_items_details)),
...
//     if (!$sponsor) {
//         //New Logic Start
//          $paymentInformation = DB::table('by_pay_informations')->where('student_id', $invoiceDetails->student_id)->latest('id')->first();
...
//             $most_recent_s_excess = 0;
//         }
//         $excessaddamount = $amount + ($most_recent_s_excess + $most_recent_h_excess);
...
//         $amountWithExcess = number_format($amount + $student_excess_amount, 2, '.', '');
...
//  $advance_amount = $amount-  $invoiceDetails->invoice_pending_amount  ;
...
//         // Check sponsor's funds
//         if ($invoiceDetails->fees_cat == 'school') {
//             if ($sponsorUser->excess_amount < $amount) {
...
//         } else {
//             if ($sponsorUser->h_excess_amount < $amount) {
//                 return response()->json(['message' => 'Sponsor has insufficient funds.'], 422);
//             }
...
//         'paid_amount' => number_format(($invoiceDetails->paid_amount ?? 0) + $payed_amount, 2, '.', ''), // Correctly add paid amount
...
//         's_excess_amount' => $s_excess_amount ?? null,
//         'h_excess_amount' => $h_excess_amount ?? null,
//     ]);
...
//     // Generate a random transaction ID
//     // $transactionId = randomId();
// // $transactionId = FastInvoiceHelper::generateReceipt();
...
Log::info('1.invoiceDetails', ['invoiceDetails' => $invoiceDetails]);
    ...
    if (!$student) {
        ...
        $paymentInformation = DB::table('by_pay_informations')->where('student_id', $invoiceDetails->student_id)->where('type', $invoiceDetails->fees_cat)->latest('id')->first();
        Log::info('1.2.paymentInformation', ['paymentInformation' => $paymentInformation]);
        Log::info('1.2.due_amount', ['due_amount' => $paymentInformation->due_amount]);
...
Log::info('2.sponsorUser', ['sponsorUser' => $sponsorUser]);
        ...
        if (!$sponsorUser) {
            ...
            Log::info('3.amount', ['amount' => $amount,'most_recent_dues' => $most_recent_dues]);
        ...
        $hostel_excess = 0;
          if ($invoiceDetails->fees_cat == 'school') {
              if($excessaddamount > $most_recent_dues){
                $school_excess= $excessaddamount - $most_recent_dues;
                $hostel_excess = 0;
            Log::info('2097.school_excess', ['excessaddamount' => $excessaddamount,'most_recent_dues' => $most_recent_dues,'school_excess' => $school_excess]);
 ...
 GenerateInvoiceView::where('invoice_no', $invoiceDetails->invoice_no)
    ...
    $payment_order_data['internal_txn_id'] = $transactionId;
                ...
                $student_excess_amount = $student->excess_amount ?? 0;
                    ...
                    $student_excess_amount = $student->h_excess_amount ?? 0;
                    User::where('id', $invoiceDetails->student_id)->update(['h_excess_amount' => ($student_excess_amount + $hostel_excess)]);
                }
                // $studentupdate = User::find($invoiceDetails->student_id);
                // $remainingexcess = ($student_excess_amount ?? 0) + ($amount ?? 0) - ($most_recent_dues ?? 0);
                // $studentupdate->h_excess_amount = $remainingexcess;
...
public function deleteinvoicetwo(Request $request)
{
    ...
    if (isset($requestData['slno'])) {
        ...
        if ($byPayInformations->isEmpty()) {
            ...
            return response()->json(['status' => 'success', 'message' => 'Invoice deleted as no receipts were found.']);
        }
        ...
        if ($Genref->fees_cat == 'school') {
            if (!$byPayInformations[0]->sponsor) {
                $student->excess_amount = number_format(floatval($student->excess_amount ?? 0)  + $totalStudentExcess - $advance_payments, 2, '.', '');
            } else {
                $sponsor = User::find($byPayInformations[0]->sponsor);
                if ($sponsor) {
                    $sponsor->excess_amount = number_format(floatval($sponsor->excess_amount ?? 0) + $totalSponsorExcess, 2, '.', '');
                    $sponsor->save();
                }
            }
        } else {
            if (!$byPayInformations[0]->sponsor) {
                $student->h_excess_amount = number_format(floatval($student->h_excess_amount ?? 0)  + $totalStudentExcess - $advance_payments, 2, '.', '');
            } else {
                $sponsor = User::find($byPayInformations[0]->sponsor);
                if ($sponsor) {
                    $sponsor->h_excess_amount = number_format(floatval($sponsor->h_excess_amount ?? 0) + $totalSponsorExcess, 2, '.', '');
                    $sponsor->save();
                }
            }
        }
        ...
        foreach ($byPayInformations as $byPayInfo) {
            DB::table('deletedreceipts')->insert([
                ...
                'transaction_completed_status' => $invoiceDetails->transaction_completed_status,
                'created_at' => $invoiceDetails->created_at,
                'updated_at' => $invoiceDetails->updated_at
            ]);

            // Delete related records for each receipt
            DB::table('by_pay_informations')->where('transactionId', $byPayInfo->transactionId)->delete();
            PaymentOrdersDetails::where('internal_txn_id', $byPayInfo->transactionId)->delete();
            PaymentOrdersStatuses::where('clnt_txn_ref', $byPayInfo->transactionId)->delete();
            Invoice_list::where('payment_transaction_id', $byPayInfo->transactionId)->delete();
        }

        // Finally, delete the invoice and GenerateInvoiceView
        GenerateInvoiceView::where('slno', $slno)->delete();

        return response()->json(['status' => 'success', 'message' => 'Invoice and related receipts deleted successfully.']);
    }
    ...
    public function deleteinvoice(Request $request)
    {
        ...
        if ($latestInvoice) {
            ...
            $latestInvoice->save();
                    ...
                    return response()->json([
                        'status' => 'success',
                        'message' => 'Invoice and corresponding payment information deleted successfully.'
                    ]);
...
public function deleterecipt(Request $request)
{
    ...
    if (isset($requestData['transactionId'])) {
        ...
        $receipt = ByPayInformation::where('transactionId',$receiptId)->first();
        ...
        $latestReceipt = ByPayInformation::where('student_id', $receipt->student_id)->where('type', $receipt->type)
        ...
        try {
            ...
            if (isset($requestData['transactionId'])) {
                ...
                PaymentOrdersDetails::where('internal_txn_id', $transactionId)
                    ->delete();

                PaymentOrdersStatuses::where('clnt_txn_ref', $transactionId)
                    ->delete();
                ...
                if($ByPayResult != ""){
                    ...
                    $updateResult = GenerateInvoiceView::where('slno', $latestReceipt->invoice_id)
                            ->update([
                                'payment_status' => $payment_status,
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Receipt and related records deleted successfully.',
                'sponsor'=>$sponsor,
                'amt'=>$amt
            ]);
 ...
 public function deleterecipttwo(Request $request)
{
    ...
    if (isset($requestData['transactionId'])) {
        ...
        // Retrieve the record from invoice_lists
        $invoiceDetails = Invoice_list::where('payment_transaction_id', $transactionId)->first();
        if (!$invoiceDetails) {
        ...
        if (!$byPayInformations) {
            return response()->json(['message' => 'Payment information not found.'], 404);
        }

        // Retrieve GenerateInvoiceView record
        $Genref = GenerateInvoiceView::where('slno', $invoiceDetails->invoice_id)->first();
        if (!$Genref) {
            return response()->json(['message' => 'GenerateInvoiceView not found.'], 404);
        }

        // Calculate updated amounts
        $updatedPaidAmount = number_format(($Genref->paid_amount ?? 0) - ($byPayInformations->amount ?? 0), 2, '.', '');
        $updatedPendingAmount = number_format(($Genref->invoice_pending_amount ?? 0) + ($byPayInformations->amount ?? 0), 2, '.', '');

        // Update the GenerateInvoiceView record
        $updateResult = GenerateInvoiceView::where('slno', $invoiceDetails->invoice_id)
            ->update([
                'payment_status' => 'Partial Paid',
                'invoice_status' => 4, // Assuming '4' means "Paid"
        ...
        if (!$updateResult) {
            return response()->json(['message' => 'Failed to update invoice details.'], 500);
        }
        ...
        if ($Genref->fees_cat == 'school') {
            if (!$byPayInformations->sponsor) {
                ...
                $student->excess_amount = number_format(($student->excess_amount ?? 0) + ($byPayInformations->student_excess_amount ?? 0) - ($byPayInformations->advance_payment ?? 0), 2, '.', '');
            } else {
                // Update sponsor excess amount
                $sponsor = User::find($byPayInformations->sponsor);
                if ($sponsor) {
                    $sponsor->excess_amount = number_format(($sponsor->excess_amount ?? 0) + ($byPayInformations->sponsor_excess_amount ?? 0), 2, '.', '');
                    $sponsor->save();
                }
            }
        } else {
            if (!$byPayInformations->sponsor) {
                ...
                $student->h_excess_amount = number_format(($student->h_excess_amount ?? 0) +  ($byPayInformations->student_excess_amount ?? 0) - ($byPayInformations->advance_payment ?? 0), 2, '.', '');
            } else {
                // Update sponsor h_excess amount
                $sponsor = User::find($byPayInformations->sponsor);
                if ($sponsor) {
                    $sponsor->h_excess_amount = number_format(($sponsor->h_excess_amount ?? 0) + ($byPayInformations->sponsor_excess_amount ?? 0), 2, '.', '');
                    $sponsor->save();
                }
            }
        }
        ...
        DB::table('deletedreceipts')->insert([
            'user_uuid' => $invoiceDetails->user_uuid,
            'invoice_id' => $invoiceDetails->invoice_id,
            'payment_transaction_id' => $invoiceDetails->payment_transaction_id,
