...
public function getAllStaffFees()
{
    try {
        ...
        $staffFees = DB::table('staff_invoices as si')
            ...
            ->leftJoin('staff_invoice_details as sid', 'si.id', '=', 'sid.invoice_id')
            ->leftJoin('staff_payments as sp', 'si.id', '=', 'sp.invoice_id')
            ->select(
                'si.id as invoice_id',
                'si.invoice_no',
                ...
                DB::raw('(COALESCE(si.total_amount, 0) - COALESCE(SUM(sp.amount), 0)) as due_amount')
            )
        ...
        $pastInvoices = DB::table('staff_invoices')
            ->select('created_by as staff_id', DB::raw('JSON_ARRAYAGG(invoice_no) as past_invoices'))
            ->groupBy('created_by')
            ->get()
            ->keyBy('staff_id');
        ...
        $receiptNumbers = DB::table('staff_payments')
            ->select('invoice_id', DB::raw('JSON_ARRAYAGG(transaction_no) as receipt_numbers'))
            ->groupBy('invoice_id')
            ->get()
...
$staffFees->transform(function ($invoice) use ($latestDues, $pastInvoices, $receiptNumbers, $invoiceMap, $oldReceipts) {
    $invoice->latest_due = $latestDues[$invoice->staff_id]->due_amount ?? 0;
    $invoice->receipt_numbers = json_decode($receiptNumbers[$invoice->invoice_id]->receipt_numbers ?? '[]', true);

    $past = isset($pastInvoices[$invoice->staff_id])
        ? json_decode($pastInvoices[$invoice->staff_id]->past_invoices ?? '[]', true)
        : [];

    $invoice->past_receipt_numbers = collect($past)->map(function ($invoiceNo) use ($invoiceMap, $oldReceipts) {
        $invoiceId = $invoiceMap[$invoiceNo] ?? null;
        if (!$invoiceId || !isset($oldReceipts[$invoiceId])) return null;

        return [
            'invoice_no' => $invoiceNo,
            'receipt_numbers' => $oldReceipts[$invoiceId]->pluck('transaction_no')->toArray(),
        ];
    })->filter()->values();
...
public function getReceiptDetails($receiptNo) 
{
    try {
        ...
        $userMaster = DB::table('staff')->where('staff_id', $receipt->staff_roll_no)->first();
...
public function getInvoiceDetails($invoiceNo)
{
    try {
        ...
        // Fetch associated receipt numbers
       

        // Fetch user_master details from staff table if available
        $userMaster = DB::table('staff')->where('staff_id', $invoice->staff_roll_no)->first();
...
public function createInvoices(Request $request)
{
    try {
        ...
        foreach ($pendingFees->groupBy('staff_id') as $staffId => $fees) {
            ...
            $nextSequence = $lastInvoice ? (intval(substr($lastInvoice, -4)) + 1) : 1;
...
public function deleteStaffInvoiceByInvoiceNo($invoiceNo)
{
    try {
        ...
        $invoice = DB::table('staff_invoices')->where('invoice_no', $invoiceNo)->first();
        ...
        DB::commit();
        return response()->json(['message' => 'Invoice and related records deleted successfully.'], 200);
...
public function storePayment(Request $request) 
{
    try {
        ...
        foreach ($payments as $payment) {
            ...
            $invoice = DB::table('staff_invoices')->where('id', $invoiceId)->first();
            if (!$invoice) {
                return response()->json(['error' => "Invoice ID $invoiceId not found"], 404);
            }
            ...
            $newNumber = $lastReceipt ? ((int) substr($lastReceipt, -4) + 1) : 1;
            $receiptNo = "R" . date('dmY') . str_pad($newNumber, 4, '0', STR_PAD_LEFT);

            // ✅ Get latest due amount from `staff_transactions`
            $latestDue = DB::table('staff_transactions')
            ...
            $totalDueLeft = DB::table('staff_transactions')
                ->where('staff_id', $staffId)
                ->orderBy('id', 'desc')
                ->value('due_amount') ?? 0;
        ...
        DB::commit();
        return response()->json(['message' => 'Payments recorded successfully'], 201);
...
public function deleteStaffFeeWithPayments($id)
{
    try {
        ...
        $staffFee = DB::table('staff_fees_mapping')->where('id', $id)->first();
        if (!$staffFee) {
        ...
        if ($invoice) {
            ...
            DB::table('staff_payments')->where('invoice_id', $invoice->id)->delete();

            // Delete the invoice
            DB::table('staff_invoices')->where('id', $invoice->id)->delete();
        }

        // Delete the staff fee mapping entry
        DB::table('staff_fees_mapping')->where('id', $id)->delete();

        DB::commit();
        return response()->json(['message' => 'Staff fee and related payments deleted successfully'], 200);
    } catch (\Exception $e) {
        DB::rollBack();
        return response()->json(['error' => $e->getMessage()], 500);
    }
    ...
    public function storePaymentold(Request $request)
    {
        try {
            ...
            $invoice = DB::table('staff_invoices')->where('id', $request->invoice_id)->first();
            $totalPaid = DB::table('staff_payments')->where('invoice_id', $request->invoice_id)->sum('amount');

            if ($totalPaid >= $invoice->total_amount) {
                DB::table('staff_invoices')->where('id', $request->invoice_id)->update(['status' => 'paid']);
            }
    ...
    {
        try {
            $invoice = DB::table('staff_invoices')->where('id', $id)->first();
            $details = DB::table('staff_invoice_details')->where('invoice_id', $id)->get();
        // Fetch user details from the 'user' table without the password field
        $user = DB::table('user')
            ->where('id', $invoice->staff_id)
            ->select('id', 'name', 'email', 'roll_no', 'created_at', 'updated_at') // Add required fields
            ->first();
    ...
    public function mapStaffFees(Request $request)
{
    try {
        ...
        $feesDetails = is_array($request->feesDetails) ? $request->feesDetails : [];

        if (empty($staffs) || empty($feesDetails)) {
            return response()->json(['error' => 'Invalid staff or fee data'], 400);
        }
        ...
        DB::commit();
        return response()->json(['message' => 'Staff mapped to fees successfully'], 201);
...
public function updateStaffFees(Request $request, $id)
{
    try {
        ...
        if ($updated) {
            return response()->json(['message' => 'Fee mapping updated successfully'], 200);
        }
...
{
    try {
        $deleted = DB::table('staff_fees_mapping')->where('id', $id)->delete();

        if ($deleted) {
            return response()->json(['message' => 'Fee mapping deleted successfully'], 200);
        }
