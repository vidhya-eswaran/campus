...
if (isset($record[1]) && $record[1] !== "") {
 ...
 //return response()->json( $admission_no);
                ...
                $existingStudent = Student::where('roll_no', 'like', $record[0])->first();
                $existingStudentuser = User::where('roll_no', 'like', $record[0])->first();
                ...
                if ($existingStudentuser) {
                    ...
                    $studentHistory->MOTHERTONGUE = $existingStudent->MOTHERTONGUE;
                    $studentHistory->STATE = $existingStudent->STATE;
                    $studentHistory->DOB_DD_MM_YYYY = $existingStudent->DOB_DD_MM_YYYY;
                    ...
                    $studentHistory->AADHAAR_CARD_NO = $existingStudent->AADHAAR_CARD_NO;
                    $studentHistory->RATIONCARDNO = $existingStudent->RATIONCARDNO;
                   ...
                   $studentHistory->save();
                    ...
                    $existingStudent->date_form  =  isset($record[3]) ? $this->convertExcelDate($record[3]) : null;
                    $existingStudent->MOTHERTONGUE  =    $record[4] ?? null;
                    $existingStudent->STATE  =    $record[5] ?? null;
                    $existingStudent->DOB_DD_MM_YYYY  =  isset($record[6]) ? $this->convertExcelDate($record[6]) : null;
                    ...
                    $existingStudent->CASTE_CLASSIFICATION  =    $record[13] ?? null;
                    ...
                    $existingStudent->PERMANENT_HOUSENUMBER  =    $record[35] ?? null;
                    ...
                    $existingStudent->COMMUNICATION_HOUSE_NO  =    $record[41] ?? null;
                    ...
                    $existingStudent->birth_certificate_photo  =    $record[56] ?? null;
                    $existingStudent->aadhar_card_photo  =    $record[57] ?? null;
                    $existingStudent->ration_card_photo  =    $record[58] ?? null;
                    $existingStudent->community_certificate  =    $record[59] ?? null;
                    $existingStudent->slip_photo  =    $record[60] ?? null;
                    $existingStudent->medical_certificate_photo  =    $record[61] ?? null;
                    $existingStudent->reference_letter_photo  =    $record[62] ?? null;
                    $existingStudent->church_certificate_photo  =    $record[63] ?? null;
                    $existingStudent->transfer_certificate_photo  =    $record[64] ?? null;
                    ...
                    $existingStudent->second_language_school  =    $record[77] ?? null;
                    ...
                    $existingStudent->guardian_organization  =    $record[84] ?? null;
        ...
        [
            ...
            'section' => $existingStudentuser->sec,
                    ...
                    $response['duplicates'][] = [
                        ...
                        'message' => 'Data modified successfully as it is already exists.'
                    ];
                } else {
                    ...
                    $student->date_form  =   isset($record[3]) ? $this->convertExcelDate($record[3]) : null;
                    $student->MOTHERTONGUE  =    $record[4] ?? null;
                    $student->STATE  =    $record[5] ?? null;
                    $student->DOB_DD_MM_YYYY  = isset($record[6]) ? $this->convertExcelDate($record[6]) : null;
                    ...
                    $student->medical_certificate_photo  =    $record[61] ?? null;
                    $student->reference_letter_photo  =    $record[62] ?? null;
                    $student->church_certificate_photo  =    $record[63] ?? null;
                    $student->transfer_certificate_photo  =    $record[64] ?? null;
                    ...
                    $student->save();
                        ...
                        Log::error('Error occurred while uploading data: ' . $e->getMessage());
                        $response['uploaded'][] = [
                            'email' => $recordEmail,
                            'admission_no' => $recordAdmissionNo,
...
//    ->whereRaw('LENGTH(admission_no) = 12')
 ...
 $lastAdmissionNo = User::where('admission_no', 'like', '%SV%')
    ->whereRaw('LENGTH(admission_no) = 12')
    ->orderByRaw("STR_TO_DATE(SUBSTRING(admission_no, 3, 6), '%d%m%y') DESC")
    ->orderByRaw("CAST(SUBSTRING(admission_no, 9, 4) AS UNSIGNED) DESC")
    ->first();
                ...
                if ($lastAdmissionNo) {
                    ...
                    $lastNumber = intval(substr($lastAdmissionNo->admission_no, 8));
...
public function readnotinuseaftersearch(Request $request)
{
    ...
    if ($limit === null) {
        ...
        $students = Student::skip($start)->take($limit)->get();
    }
...
{   
    $fields = [
        'id', 'roll_no', 'admission_no', 'STUDENT_NAME', 'date_form', 'MOTHERTONGUE',
        'STATE', 'DOB_DD_MM_YYYY', 'SEX', 'BLOOD_GROUP', 'NATIONALITY', 'RELIGION',
        'DENOMINATION', 'CASTE', 'CASTE_CLASSIFICATION', 'AADHAAR_CARD_NO', 'RATIONCARDNO',
        'EMIS_NO', 'FOOD', 'chronic_des', 'medicine_taken', 'FATHER', 'OCCUPATION', 'MOTHER',
        'mother_occupation', 'GUARDIAN', 'guardian_occupation', 'MOBILE_NUMBER', 'EMAIL_ID',
        'WHATS_APP_NO', 'mother_email_id', 'guardian_contact_no', 'guardian_email_id',
        'MONTHLY_INCOME', 'mother_income', 'guardian_income', 'PERMANENT_HOUSENUMBER',
        'P_STREETNAME', 'P_VILLAGE_TOWN_NAME', 'P_DISTRICT', 'P_STATE', 'P_PINCODE',
        'COMMUNICATION_HOUSE_NO', 'C_STREET_NAME', 'C_VILLAGE_TOWN_NAME', 'C_DISTRICT',
        'C_STATE', 'C_PINCODE', 'CLASS_LAST_STUDIED', 'NAME_OF_SCHOOL', 'SOUGHT_STD', 'sec',
        'syllabus', 'GROUP_12', 'group_no', 'second_group_no', 'LANG_PART_I', 'profile_photo',
        'birth_certificate_photo', 'aadhar_card_photo', 'ration_card_photo',
        'community_certificate', 'slip_photo', 'medical_certificate_photo',
        'reference_letter_photo', 'church_certificate_photo', 'transfer_certificate_photo',
        'admission_photo', 'payment_order_id', 'siblings', 'brother_1', 'brother_2',
        'gender_1', 'gender_2', 'class_1', 'class_2', 'brother_3', 'gender_3', 'class_3',
        'last_school_state', 'second_language_school', 'second_language', 'reference_name_1',
        'reference_name_2', 'reference_phone_1', 'reference_phone_2', 'ORGANISATION',
        'mother_organization', 'guardian_organization', 'academic_year', 'grade_status', 'created_at', 'updated_at',
        'documents', 'admission_id', 'father_title', 'mother_title', 'guardian_title',
        'status', 'upload_created_at', 'upload_updated_at'
    ];
    ...
    foreach ($request->except(['limit', 'start', 'search']) as $field => $value) {
    ...
    $students = $request->has('limit')
        ? $query->skip($request->input('start', 0))->take($request->input('limit'))->get()
        : $query->get();
...
$rollNos = $students->pluck('roll_no')->toArray();
 // Convert to array
    ...
    return response()->json(['students' => $students, 'total' => $total, 'excelLink' => $downloadUrl]);
}
   ...
   {   
    ...
    public function readByadmissioNo($admission_no)
    {
        ...
        if ($students) {
            ...
            $imagePath = storage_path('app/public/photos/' . $admission_no . '.jpg');
    ...
    public function update(Request $request, $id)
{
 ...
 //   $admitted_student = Student::findOrFail($admission->admitt);
    ...
    // Handle file uploads for image fields
        $this->handleImageUpdate($request, $admission, 'migration_certificate','admission_photos','admission_photo');
    $this->handleImageUpdate($request, $admission, 'profile_photo','profile_photos','profile_photo');
    $this->handleImageUpdate($request, $admission, 'admission_photo','admission_photos','admission_photo');
    $this->handleImageUpdate($request, $admission, 'birth_certificate','birth_certificate_photos','birth_certificate_photo');
    $this->handleImageUpdate($request, $admission, 'aadhar_copy','aadhar_card_photos','aadhar_card_photo');
    $this->handleImageUpdate($request, $admission, 'ration_card','ration_card_photos','ration_card_photo');
    $this->handleImageUpdate($request, $admission, 'community_certificate','community_certificate_photos','community_certificate');
    $this->handleImageUpdate($request, $admission, 'salary_certificate','slip_photos','slip_photo');
    $this->handleImageUpdate($request, $admission, 'medical_certificate','medical_certificate_photos','medical_certificate_photo');
    $this->handleImageUpdate($request, $admission, 'reference_letter','reference_letter_photos','reference_letter_photo');
    $this->handleImageUpdate($request, $admission, 'church_certificate','church_certificate_photos','church_certificate_photo');
    $this->handleImageUpdate($request, $admission, 'transfer_certificate','transfer_certificate_photos','transfer_certificate_photo');
    if (strtolower((string) $request->status) === 'active') {
        ...
        if (!$admission->roll_no) {
            ...
            $lastAdmissionNo = Student::where('roll_no', 'like', $baseAdmissionNo . '%')
                ->orderBy('admission_no', 'desc')
                ->value('roll_no');
    ...
    ->first();
                ...
                $admissionId =  $newAdmissionNo;
                       ...
                       if (!User::where('roll_no', $admission->roll_no)
                    ...
                    try {
                        $userId = $user 
                            ? $user->id 
                            : User::where('roll_no', $admission->roll_no)->latest('id')->value('id');
                        ...
                        \Log::error('Failed to log application active lifecycle.  Status Updated to Active', [
                            'user_id' => $userId ?? 'N/A',
                            'error' => $e->getMessage(),
                        ]);
                    }
           ...
           try {
                        $userId =  User::where('roll_no', $admission->roll_no)->latest('id')->value('id');
                        ...
                        \Log::error('Failed to log application active lifecycle.  Status Updated record', [
                            'user_id' => $userId ?? 'N/A',
                            'error' => $e->getMessage(),
                        ]);
                    }  
...
Log::info('Request payload:', $request->all());
    ...
    $admission->update([
      //  'admission_no' => $request->input('admission_no'),
        ...
        'second_language_school' => $request->input('second_language_school'),
...
foreach (['profile_photo', 'admission_photo', 'birth_certificate', 'aadhar_copy','ration_card','community_certificate','salary_certificate','medical_certificate','reference_letter','church_certificate','transfer_certificate'] as $field) {
  ...
  //  request,$dmission, feildname on api,filepath,dbname
          $this->handleImageUpdate($request, $admission, 'migration_certificate','admission_photos','admission_photo');
    $this->handleImageUpdate($request, $admission, 'profile_photo','profile_photos','profile_photo');
    $this->handleImageUpdate($request, $admission, 'admission_photo','profile_photos','admission_photo');
    $this->handleImageUpdate($request, $admission, 'birth_certificate','birth_certificate_photos','birth_certificate_photo');
    $this->handleImageUpdate($request, $admission, 'aadhar_copy','aadhar_card_photos','aadhar_card_photo');
    $this->handleImageUpdate($request, $admission, 'ration_card','ration_card_photos','ration_card_photo');
    $this->handleImageUpdate($request, $admission, 'community_certificate','community_certificate_photos','community_certificate');
  ...
  try{
    ...
    return response()->json([
        ...
        'data' => [
            ...
            'mother_income' => $admission->mother_income,
            'guardian_income' => $admission->guardian_income,
            'PERMANENT_HOUSENUMBER' => $admission->PERMANENT_HOUSENUMBER,
            ...
            'COMMUNICATION_HOUSE_NO' => $admission->COMMUNICATION_HOUSE_NO,
        ...
        return response()->json([
            'message' => 'An error occurred while fetching the admission record.',
            'error' => $e->getMessage()
        ], 500);
    }
}
...
try {
    ...
    return response()->json([
        'message' => 'Admission submitted details fetched successfully!',
        'data' => [
            'roll_no' => '',
            'admission_no' => $admission->admission_no,
            'STUDENT_NAME' => $admission->name,
...
private function handleImageUpdate($request, $staff, $fieldNameApi, $filePath, $dbName)
{
    try {
        if ($request->has($fieldNameApi)) {
            ...
            if (preg_match('/^data:image\/(\w+);base64,/', $base64Image, $matches)) {
                ...
                if ($isWritten === false) {
                    throw new \Exception("Failed to write file for field: $fieldNameApi");
                }

                // Store the file name in the database, not the base64 string
                $staff->update([$dbName => $fileName]);

                // Log success
                Log::info("Successfully updated {$fieldNameApi} with file path: {$filePathWithName}");
            } else {
...
private function handleImageUpdatepreviosu($request, $admission, $fieldName)
{
    ...
    if ($request->hasFile($fieldName)) {
     ...
     $filename = $fieldName . $admission->id . '_' . $timestamp . '.' . $extension;

        $path = $request->$fieldName->storeAs($fieldName . '_photos', $filename);
        ...
        Storage::disk('local')->put($fieldName . '_photos/' . $path, $imageData);
 ...
 public function updatetttt(Request $request, $id)
    {
        ...
        $record->CongenitalHeartDisease = $request->input('CongenitalHeartDisease');
        ...
        $record->permission_to_principal = $request->input('permission_to_principal');
        $record->administration_of_anaesthetic = $request->input('administration_of_anaesthetic');
        ...
        $record->second_language_school = $request->input('second_language_school');
...
{
     $admission = Student::findOrFail($id);
   if ($admission->profile_photo && Storage::exists($admission->profile_photo)) {
        Storage::delete($admission->profile_photo); // Delete the profile photo
    }

    if ($admission->admission_photo && Storage::exists($admission->admission_photo)) {
