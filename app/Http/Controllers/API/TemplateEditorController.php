...
public function bonafideView(Request $request)
{
    ...
    $data = [
        ...
        'date_of_birth_words' => $this->convertDateToWords($student->DOB_DD_MM_YYYY),
        'admission_date' => $student->date_form,
    ];
 
    $url = $this->generateCertificate($student, $template, $data, 'Bonafide Certificate', 'bonafide_certificate');
...
public function courseCompletion(Request $request) 
{
    ...
    $url = $this->generateCertificate($student, $template, $data, 'Course Completion Certificate', 'certificate');
 ...
 public function idcard(Request $request)  
{
    ...
    if (!$request->has('student_ids') || !is_array($request->student_ids)) {
        return response()->json(['message' => 'Please provide an array of student IDs'], 400);
    }
    ...
    foreach ($request->student_ids as $studentId) {
        ...
        $filename = 'idcard_' . Str::slug($student->STUDENT_NAME) . '_' . $studentId;
        // Generate the certificate with custom filename
        $url = $this->generateCertificateid($student, $template, $data, 'Identity Card', $filename);
...
private function generateCertificateid($student, $template, $data, $title, $filenamePrefix)
{
    ...
    $html = preg_replace_callback('/class="([^"]*)"/', function ($matches) {
        ...
        $classes = array_filter($classes, fn($cls) => $cls !== 'hidden-text');
...
public function noDue(Request $request) 
{
    ...
    // Generate the certificate (assuming this function handles PDF creation and returns URL)
    $url = $this->generateCertificate($student, $template, $data, 'Nodue', 'certificate');
...
public function studentAttendanceView(Request $request)
{
    ...
    $url = $this->generateCertificate($student, $template, $data, 'Attendance Certificate', 'attendance_certificate');
...
public function studentContactView(Request $request) 
{
    ...
    $url = $this->generateCertificate($student, $template, $template_data, 'Student Certificate', 'certificate');

    return response()->json(['url' => $url], 200);
}
 

private function generateCertificate($student, $template, $data, $title, $filePrefix)
{
...
$body = preg_replace_callback('/class="([^"]*)"/', function ($matches) {
    $classes = preg_split('/\s+/', trim($matches[1]));
    $classes = array_filter($classes, fn($cls) => $cls !== 'hidden-text');
    return 'class="' . implode(' ', $classes) . '"';
}, $body);
...
HTML;
    ...
    $uploadPath = $_SERVER['DOCUMENT_ROOT'] . '/' . $folderName . '/public/certificates';
...
private function convertNumberToWords($number) {
    ...
    if ($number < 20) {
        ...
        $ones = $number % 10;
        return $ones > 0 ? $words[$tens] . '-' . $words[$ones] : $words[$tens];
    } elseif ($number < 1000) {
        $hundreds = floor($number / 100);
        $remainder = $number % 100;
        return $words[$hundreds] . ' hundred' . ($remainder > 0 ? ' and ' . $this->convertNumberToWords($remainder) : '');
    } elseif ($number < 100000) {
        $thousands = floor($number / 1000);
        $remainder = $number % 1000;
        return $this->convertNumberToWords($thousands) . ' thousand' . ($remainder > 0 ? ' ' . $this->convertNumberToWords($remainder) : '');
    } elseif ($number < 10000000) {
        $lakhs = floor($number / 100000);
        $remainder = $number % 100000;
        return $this->convertNumberToWords($lakhs) . ' lakh' . ($remainder > 0 ? ' ' . $this->convertNumberToWords($remainder) : '');
    } else {
        $crores = floor($number / 10000000);
        $remainder = $number % 10000000;
        return $this->convertNumberToWords($crores) . ' crore' . ($remainder > 0 ? ' ' . $this->convertNumberToWords($remainder) : '');
    }
}
...
private function replaceTemplateVariables($templateString, $data)
{
    ...
    foreach ($data as $key => $value) {
        ...
        $count = 0;
        $templateString = preg_replace($pattern, $value, $templateString, -1, $count);
...
// }
    ...
    public function previewPdf($id)
    {
        ...
        $template = TemplateEditor::where('template_name', 'Attendance Certificate')->first();
