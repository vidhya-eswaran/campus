...
{
    //Select Class*:
    // ReceiptorInvoice FromDate ToDate Studentid =>
    //Invoiceid Recieptsid =>
    // AdmissionNo =   FromDate ToDate
    ...
    { 
        ...
        if (!$request->has('FromDate') || !$request->has('ToDate')) {
            ...
            return response()->json(['error' => 'FromDate and ToDate are required.'], 400);
        }
        $requestData =  $request->all();
$responseData = ['H' => [], 'S' => []];

//  return response()->json(['error' => $requestData], 500);
if ($request->has('AdmissionNo') && $request->input('AdmissionNo') !== null) {
            $admissionNo = $request->input('AdmissionNo');
            $user = User::where('id', $admissionNo)->first();
             $student = Student::where('roll_no', $user->roll_no)
    ->orderBy('created_at', 'desc')
    ->first();

            if (!$user && (!$request->has('Recieptsid') || empty($request->input('Recieptsid'))) && (!$request->has('Invoiceid') || empty($request->input('Invoiceid')))) {
        ...
        if ($request->has('FromDate') && $request->has('ToDate')) {
            ...
            if ($request->has('ReceiptorInvoice') && !empty($request->input('ReceiptorInvoice')) && $request->input('ReceiptorInvoice') === 'invoice' && !empty($userId)) {
                $invoiceRecordsO = GenerateInvoiceView::where('created_at', '>=', $fromDate)
                     ->where('created_at', '<=', $toDate)
                    ->where('student_id', $userId)->where('fees_cat','other')
                    ->get();
...
if (isset($sponsorValue) && !empty($sponsorValue)) {
                $sponsorUser = User::where('id', $sponsorValue)->first();
    ...
    $responseData['H'][] = [
        ...
        'studentmaster' => $student,
                  ...
                  $invoiceRecordsS = GenerateInvoiceView::where('created_at', '>=', $fromDate)
    ->where('created_at', '<=', $toDate)
                    ->where('student_id', $userId)->where('fees_cat','school')
                    ->get();
...
foreach ($invoiceRecordsS as $invoice) {
    ...
    $responseData['S'][] = [
        ...
        'studentmaster' => $student,
            ...
            } elseif ($request->has('ReceiptorInvoice') && $request->input('ReceiptorInvoice') === 'receipt' && !empty($userId)) {
                // Filter ByPayInformation records based on the date range and student_id
                $receipts = ByPayInformation::where('created_at', '>=', $fromDate)
                     ->where('created_at', '<=', $toDate)
                    ->where('student_id', $userId)
                    ->get();

                // Get unique invoice IDs from the filtered receipts
                $invoiceIds = $receipts->pluck('invoice_id')->unique()->toArray();

                // Fetch GenerateInvoiceView records associated with the invoice IDs
                $invoiceRecordsS = GenerateInvoiceView::whereIn('slno', $invoiceIds)->where('fees_cat','school')
                    ->get();
    ...
    $filteredReceipts = $receipts->where('invoice_id', $invoice->slno)->values()->all();
    ...
    $responseData['S'][] = [ // Changed from $responseData['S'] = to $responseData['S'][] = to ensure multiple entries
        'invoice' => $invoice,
        ...
        'studentmaster' => $student,
                 ...
                 $invoiceRecordsO = GenerateInvoiceView::whereIn('slno', $invoiceIds)->where('fees_cat','other')
                    ->get();
...
foreach ($invoiceRecordsO as $invoice) {
    ...
    // Add the invoice and its associated receipts to the response data
    $responseData['H'][] = [ // Use [] to append instead of overwriting
        'invoice' => $invoice,
        'receipts' => $filteredReceipts, // If no receipts exist, this will be an empty array
        'student' => $user,
        'studentmaster' => $student,
...
if ($request->filled('SponsorID')) {
    ...
    $invoiceRecordsh->load(['byPayInformations' => function ($query) use ($sponsorId) {
...
$filteredByPayInformations = $byPayInformations->filter(function ($byPayInformation) use ($sponsorId) {
    ...
    if ($sponsorId) {
        ...
        return $byPayInformation->sponsor !== null || $byPayInformation->inv_amt !== null;
    }
  ...
  $filteredByPayInformations  =$byPayInformations ; 
            ...
            foreach ($filteredByPayInformations as $byPayInformation) {
                $sponsorValue = $byPayInformation->sponsor;
            ...
            $tempData['H'][] = [
                'invoice' => $invoice,
                'receipts' => $filteredByPayInformations->values()->all(), // Ensure this is an array
                'student' => $student,
            ];
        }
    ...
    ->where('generate_invoice_views.student_id', $student->id)
    ->whereBetween('generate_invoice_views.created_at', [$fromDate, $toDate])
    ->where('generate_invoice_views.fees_cat', 'school');

// Apply sponsor filter if provided
if ($request->filled('SponsorID')) {
    $invoiceRecordsS->whereHas('byPayInformations', function ($query) use ($request) {
        $query->where('sponsor', $request->input('SponsorID'));
    });
}

$invoiceRecordsS = $invoiceRecordsS->select('generate_invoice_views.*')
    ->distinct()
    ->get();
 ...
 $filteredPayInformations = $byPayInformations->filter(function ($payInfo) use ($sponsorId) {
    ...
    if ($sponsorId) {
        ...
        return $payInfo->sponsor !== null || $payInfo->inv_amt !== null;
    }
    // If no SponsorID is provided, include all records
    return true;
});
            ...
            if ($filteredPayInformations->isNotEmpty()) {
                ...
                $sponsorIds = $filteredPayInformations->pluck('sponsor')->filter()->unique();

                // Fetch all sponsors at once
                $sponsorUsers = User::whereIn('id', $sponsorIds)->get()->keyBy('id');

                // Add sponsor information to each pay information
                foreach ($filteredPayInformations as $byPayInformation) {
                                    $byPayInformation->student = $student;

                    if ($byPayInformation->sponsor && isset($sponsorUsers[$byPayInformation->sponsor])) {
                        $byPayInformation->sponsor_info = $sponsorUsers[$byPayInformation->sponsor];
                    }
                }
            }
            ...
            $tempData['S'][] = [
                ...
                'receipts' => $filteredPayInformations->values()->all(), // Ensure this is an array
                'student' => $student,
            ];
        }
    }
...
if ($receiptOrInvoice === 'receipt') {
    ...
    if ($request->filled('SponsorID')) {
        $receiptRecordsO->where('sponsor', $request->input('SponsorID'));
    }

    // Fetch all 'other' receipts and attach student and sponsor info
    $receiptRecordsO->get()->each(function ($receipt) use ($student, &$tempData) {
        $receipt->student = $student; // Attach student info to the receipt
        if ($receipt->sponsor) {
            $receipt->sponsor_info = User::find($receipt->sponsor); // Fetch sponsor info
        }
    $invoice = GenerateInvoiceView::where('slno', $receipt->invoice_id) ->first();
    ...
    if ($request->filled('SponsorID')) {
        $receiptRecordsS->where('sponsor', $request->input('SponsorID'));
    }

    // Fetch all 'school' receipts and attach student and sponsor info
    $receiptRecordsS->get()->each(function ($receipt) use ($student, &$tempData) {
        $receipt->student = $student; // Attach student info to the receipt
        if ($receipt->sponsor) {
            $receipt->sponsor_info = User::find($receipt->sponsor); // Fetch sponsor info
        }
...
$responseData['H'] = is_array($responseData['H']) ? $responseData['H'] : (array)$responseData['H'];
$responseData['S'] = is_array($responseData['S']) ? $responseData['S'] : (array)$responseData['S'];

// Merge with $tempData
$responseData['H'] = array_merge($responseData['H'], is_array($tempData['H']) ? $tempData['H'] : (array)$tempData['H']);
$responseData['S'] = array_merge($responseData['S'], is_array($tempData['S']) ? $tempData['S'] : (array)$tempData['S']);

}

    return response()->json(['data' => $responseData, 'ddf' => 33], 200);
}
                            ...
                            foreach ($students as $student) {
                                if ($receiptOrInvoice === 'invoice' || $receiptOrInvoice === 'both') {
                                    // Fetch invoice records for 'other' fees
                                    $invoiceRecordsO = GenerateInvoiceView::where('student_id', $student->id)
                                        ->where('fees_cat', 'other');
                                        // Check if SponsorID is provided and not empty
                    if ($request->has('SponsorID') && !empty($request->input('SponsorID'))) {
                        // Add sponsor filter to the query
                        $invoiceRecordsO->whereHas('byPayInformations', function ($query) use ($request) {
                            $query->where('sponsor', $request->input('SponsorID'));
                        });
                    }
                           ...
                           $invoiceRecordsO=$invoiceRecordsO->get();
                                    ...
                                    // Fetch invoice records for 'school' fees
                                    $invoiceRecordsS = GenerateInvoiceView::where('student_id', $student->id)
                                ...
                                if ($receiptOrInvoice === 'receipt' || $receiptOrInvoice === 'both') {
                                    // Fetch receipt records for 'other' fees
                                    $receiptRecordsO = ByPayInformation::where('student_id', $student->id)
                                        ->where('fees_cat', 'other');
                                        // Check if SponsorID is provided and not empty
                    if ($request->has('SponsorID') && !empty($request->input('SponsorID'))) {
                        // Add sponsor filter to the query
                        $receiptRecordsO->where('sponsor', $request->input('SponsorID'));
                                    ...
                                    // Fetch receipt records for 'school' fees
                                    $receiptRecordsS = ByPayInformation::where('student_id', $student->id)
                                        ->where('fees_cat', 'school')
                                        ->get();
            ...
            elseif ($request->has('Recieptsid') && !empty($request->input('Recieptsid'))) {
                // Filter ByPayInformation records based on the date range and student_id
                $receipts = ByPayInformation::where('transactionId', $request->input('Recieptsid'))
                    ->first();
                ...
                // Fetch GenerateInvoiceView records associated with the invoice IDs
                $invoiceRecords = GenerateInvoiceView::where('slno', $invoiceIds)
                    ->first();
                ...
                // }
                $studentinfos = User::where('id', $invoiceRecords->student_id)->first();
                ...
                // Return the response
                return response()->json(['data' => $responseData], 200);
            } elseif ($request->has('Invoiceid') && !empty($request->input('Invoiceid'))) {
                $invoiceRecords = GenerateInvoiceView::where('invoice_no', $request->input('Invoiceid'))
                    ->get();
                ...
                foreach ($invoiceRecords as $invoice) {
                    ...
                    $studentinfos = User::where('id', $invoice->student_id)->first();
                ...
                // Return the response after processing all the invoice records
                return response()->json(['data' => $responseData], 200);
            } elseif ($request->has('SponsorIDtwo') && !empty($request->input('SponsorIDtwo'))) {
                // Filter ByPayInformation records based on the date range and student_id
$query = ByPayInformation::where('sponsor', $request->input('SponsorID'));
...
$invoiceRecordsforspons = GenerateInvoiceView::where('sponser_id', $request->input('SponsorID'))
   ->where('created_at', '>=', $fromDate) // Include fromDate
    ->where('created_at', '<=', $toDate) ->get();

// Merge the invoices from both queries
$mergedInvoices = $invoiceRecords->merge($invoiceRecordsforspons)->unique('slno');
...
foreach ($mergedInvoices as $invoice) {
    // Filter receipts associated with the current invoice
    $filteredReceipts = $receipts->where('invoice_id', $invoice->slno)->values();
...
return response()->json(['data' => $responseData], 200);
            ...
            } elseif ($request->has('StudentID') && !empty($request->input('StudentID'))) {
                $studentID = $request->input('StudentID');

                //     FromDate": "2024-01-01",
                // "ToDate": "2024-01-03",
                // "StudentID": "67"
                $totalInvoiceAmounts = GenerateInvoiceView::where('student_id', $studentID)
                                    ->whereBetween('created_at', [$request->input('FromDate'), $request->input('ToDate')])
                                    ->get();
                $schoolslnos = GenerateInvoiceView::where('student_id', $studentID)
                    ->where('fees_cat', 'school')
                    ->whereBetween('created_at', [$request->input('FromDate'), $request->input('ToDate')])
                    ->select('slno')
                    ->get()
                 ...
                 $hostelslnos = GenerateInvoiceView::where('student_id', $studentID)
                    ->where('fees_cat', 'other')
                    ->whereBetween('created_at', [$request->input('FromDate'), $request->input('ToDate')])
                    ->select('slno')
                    ->get()
                ...
                $schoolbyPayInformation = ByPayInformation::whereNull('inv_amt')->whereIn('invoice_id', $schoolslnos)->get();
                $schooltotalByPayAmount = $schoolbyPayInformation->sum('amount');
                $hostelbyPayInformation = ByPayInformation::whereNull('inv_amt')->whereIn('invoice_id', $hostelslnos)->get();
                $hosteltotalByPayAmount = $hostelbyPayInformation->sum('amount');
                $schooltotals = [];
                $hosteltotals = [];
                $distotals = [];
                ...
                foreach ($totalInvoiceAmounts as $invoice) {
                    ...
                    $schooltotalPaymentAmount = $schooltotalPayments[$invoice->slno] ?? 0;
                    $hosteltotalPaymentAmount = $hosteltotalPayments[$invoice->slno] ?? 0;
                    if($invoice->fees_cat == 'school'){
                     foreach ($feesItemsDetails as $item) {
                        ...
                        $feesHeading = $item['fees_heading'];
                          $schooltotals['invoice'][$feesHeading][$feesSubHeading] = isset($schooltotals['invoice'][$feesHeading][$feesSubHeading]) ? $schooltotals['invoice'][$feesHeading][$feesSubHeading] + $feesAmount : $feesAmount;
                        ...
                        if ($schooltotalPaymentAmount >= $feesAmount) {
                            ...
                            if (!isset($schooltotals['recpt'][$feesHeading])) {
                                $schooltotals['recpt'][$feesHeading] = [];
                            }

                            // Add the fees amount to the fees subheading under the fees heading
                            $schooltotals['recpt'][$feesHeading][$feesSubHeading] = ($schooltotals['recpt'][$feesHeading][$feesSubHeading] ?? 0) + $feesAmount;
                            ...
                            $schooltotals['recpt'][$feesHeading][$feesSubHeading] = ($schooltotals['recpt'][$feesHeading][$feesSubHeading] ?? 0) + $schooltotalPaymentAmount;
                     ...
                     foreach ($feesItemsDetails as $item) {
                        ...
                        $feesAmount = $item['amount'];
                        $feesHeading = $item['fees_heading'];
                          $hosteltotals['invoice'][$feesHeading][$feesSubHeading] = isset($hosteltotals['invoice'][$feesHeading][$feesSubHeading]) ? $hosteltotals['invoice'][$feesHeading][$feesSubHeading] + $feesAmount : $feesAmount;

                        // If the total payment amount is greater than or equal to the fees amount,
                        // allocate the fees amount to the corresponding subheading
                        if ($hosteltotalPaymentAmount >= $feesAmount) {
                            // If the fees heading doesn't exist in the recpt array, initialize it
                            if (!isset($hosteltotals['recpt'][$feesHeading])) {
                                $hosteltotals['recpt'][$feesHeading] = [];
                            }

                            // Add the fees amount to the fees subheading under the fees heading
                            $hosteltotals['recpt'][$feesHeading][$feesSubHeading] = ($hosteltotals['recpt'][$feesHeading][$feesSubHeading] ?? 0) + $feesAmount;
                            ...
                            // Add the remaining payment amount to the fees subheading under the fees heading
                            $hosteltotals['recpt'][$feesHeading][$feesSubHeading] = ($hosteltotals['recpt'][$feesHeading][$feesSubHeading] ?? 0) + $hosteltotalPaymentAmount;
                ...
                if($invoice->fees_cat == 'school'){
                        foreach ($discountItemsDetails as $disitem) {
                            ...
                            $distotals['schooldiscount'][$discountCat] = ($distotals['schooldiscount'][$discountCat] ?? 0) + $discountAmount;
                          $schooltotaDiscountAmount += $discountAmount;
                        }
                        $schooltotalInvoiceAmount += $invoice->total_invoice_amount??0;
                        $schoolpaymentInformation = DB::table('by_pay_informations')->where('invoice_id', $invoice->slno)->where('type', $invoice->fees_cat)->latest('id')->first();
                        $schooltotaldues += $schoolpaymentInformation->due_amount??0;
                        $schooltotalexcess += $schoolpaymentInformation->s_excess_amount??0;
                    } else {
                        foreach ($discountItemsDetails as $disitem) {
                        $discountCat = $disitem['discount_cat'];
                        $discountAmount = $disitem['dis_amount'];
                        //  $distotals['disrecpt'][$discountCat] = [];
                        //  $distotals['disrecpt'][$discountCat] = ($distotals['disrecpt'][$discountCat] ?? 0) + $discountAmount;
                        $distotals['hosteldiscount'][$discountCat] = ($distotals['hosteldiscount'][$discountCat] ?? 0) + $discountAmount;
                        $hosteltotaDiscountAmount += $discountAmount;
                    }
                    $hosteltotalInvoiceAmount += $invoice->total_invoice_amount??0;
                    $hostelpaymentInformation = DB::table('by_pay_informations')->where('invoice_id', $invoice->slno)->where('type', $invoice->fees_cat)->latest('id')->first();
                    $hosteltotaldues += $hostelpaymentInformation->due_amount??0;
                    $hosteltotalexcess += $hostelpaymentInformation->h_excess_amount??0;
                    }
                }
                

                // Logic to calculate total invoice amount
                $totalInvoiceAmount = GenerateInvoiceView::where('student_id', $studentID)
                    ->whereBetween('created_at', [$request->input('FromDate'), $request->input('ToDate')])
                ...
                $userDataMaster = Student::where('admission_no', $userData->admission_no)->first();
                ...
                return response()->json(['error' => 'StudentID not provided or empty.'], 400);
            }
        } else {
            // Handle case where either fromDateDate or ToDate is missing
            return response()->json(['error' => 'Both FromDate and ToDate are required.'], 400);
        }
    ...
    public function LedgerSummary(Request $request)
    {
        ...
        //             ], 200);
        if ($request->has('FromDate') && $request->has('ToDate')) {
            $fromDate =  Carbon::parse($request->input('FromDate'))->startOfDay();  // '00:00:00' of the start day
        ...
        //   dd($request->Std);
            ...
            // if ($request->has('Std') && !empty($request->input('Std'))) {
                ...
                if($request->Std != 'All' && $request->has('Std') && !empty($request->input('Std'))){
                //   if ($request->has('Std') && !empty($request->input('Std'))) {
                $totalInvoiceAmounts = GenerateInvoiceView::where('standard', $Std)
                    ->where('created_at', '>=', $fromDate)
                     ->where('created_at', '<=', $toDate)
                    ->get();
                } else {
                    $totalInvoiceAmounts = GenerateInvoiceView::where('created_at', '>=', $fromDate)
                     ->where('created_at', '<=', $toDate)
                    ->get();
                }
                ...
                foreach ($totalInvoiceAmounts as $invoiceAmounts) {
                    $feesItemsDetails = json_decode($invoiceAmounts->fees_items_details, true);
                    ...
                    foreach ($feesItemsDetails as $item) {
                        ...
                        // Update the total invoice amount
                            $totals['invoice'][$feesHeading][$feesSubHeading] = isset($totals['invoice'][$feesHeading][$feesSubHeading]) ? $totals['invoice'][$feesHeading][$feesSubHeading] + $amount : $amount;
                    }
                ...
                if ($request->has('SponsorID') && !empty($request->input('SponsorID'))) {
                    $payments = ByPayInformation::whereIn('invoice_id', $totalInvoiceAmounts->pluck('slno'))->where('sponsor', $request->input('SponsorID'))->get();
                } else {
                    $payments = ByPayInformation::whereIn('invoice_id', $totalInvoiceAmounts->pluck('slno'))->get();
                }
                $totalPayments = [];
               $latestPayments = ByPayInformation::whereIn('invoice_id', $totalInvoiceAmounts->pluck('slno'))
                    ->select('invoice_id', DB::raw('MAX(id) as latest_id'))
                    ->groupBy('invoice_id');
                
                // Now fetch the full records for the latest IDs
                $latestpayments = ByPayInformation::whereIn('id', $latestPayments->pluck('latest_id'))
                    ->orderBy('id', 'desc')
                    ->get();
                ...
                foreach ($totalInvoiceAmounts as $invoice) {
                    ...
                    foreach ($feesItemsDetails as $item) {
                        ...
                        $feesAmount = $item['amount'];
                        $feesHeading = $item['fees_heading'];
                          $totals['invoice'][$feesHeading][$feesSubHeading] = isset($totals['invoice'][$feesHeading][$feesSubHeading]) ? $totals['invoice'][$feesHeading][$feesSubHeading] + $feesAmount : $feesAmount;
                        ...
                        if ($totalPaymentAmount >= $feesAmount) {
                            ...
                            // Add the fees amount to the fees subheading under the fees heading
                            $totals['recpt'][$feesHeading][$feesSubHeading] = ($totals['recpt'][$feesHeading][$feesSubHeading] ?? 0) + $feesAmount;
                            ...
                            // Add the remaining payment amount to the fees subheading under the fees heading
                            $totals['recpt'][$feesHeading][$feesSubHeading] = ($totals['recpt'][$feesHeading][$feesSubHeading] ?? 0) + $totalPaymentAmount;
       ...
       public function getPaymentReport(Request $request)
    {
        ...
        $query = DB::table('by_pay_informations')
            ->select(
                'id',
                'student_id',
                'invoice_id',
  ...
  $query->whereNull('inv_amt');
   ...
   if ($sponsor) {
        ...
        $results = $results->map(function ($result) {
            $student = DB::table('users')->where('id', $result->student_id)->first(); // Fetch student data

              $dateOfReceipt = null;
            if ($result->created_at) { // Check if created_at is not null
                try {
                    $dateOfReceipt = Carbon::parse($result->created_at)->format('Y-m-d');
                } catch (\Exception $e) {
                    // Log the error or handle it as needed
                    \Log::error("Error parsing date: " . $result->created_at . " - " . $e->getMessage());
            ...
            return [
                ...
                'sponsorName' =>  optional(User::where('id', $result->sponsor)->first())->name, // Sponsor name from by_pay_informations
                'feesCategory' => $result->type, // Type from by_pay_informations, assuming it's feesCategory
                'academicYear' =>  optional(GenerateInvoiceView::where('slno', $result->invoice_id)->first())->acad_year, // Hardcoded academic year, adjust as needed
                's_excess_amount' => $this->getMostRecentExcess($result->student_id, 'school'),
                'h_excess_amount' => $this->getMostRecentExcess($result->student_id, 'other'),
            ];
        });
 ...
 $class = $request->input('class');
    ...
    ->when(!empty($sponsor), function ($query) use ($sponsor) { // Fixed function argument
        if (is_array($sponsor)) {
    ...
    ->when(!empty($studentIds), function ($query) use ($studentIds) { // Fixed `students` condition
...
$reportData = $userDatasResults->map(function ($student) use ($sponsor) {
...
// return $latestSchool;
            ...
            return [
                ...
                'School Excess' => $latestSchool ? $latestSchool->s_excess_amount : 0,
                'Hostel Dues' => $latestHostel ? $latestHostel->due_amount : 0,
                'Hostel Excess' => $latestHostel ? $latestHostel->h_excess_amount : 0,
                'Sponsor Name' => $sponsor ? optional(User::find($sponsor))->name : null,
                'Fees Category' => $latestSchool ? $latestSchool->type : null,
                'Academic Year' => $latestSchool ? optional(GenerateInvoiceView::where('slno', $latestSchool->invoice_id)->first())->acad_year : null,
            ];
        });
