...
class ClassTeacherController extends Controller
{
    ...
    //View List ClassTeacherController
//   public function index()
// {
//     $data = ClassTeacher::all()->map(function ($item) {
//         $item->class_teacher = is_string($item->class_teacher) ? json_decode($item->class_teacher, true) : $item->class_teacher;
//         $item->std_and_sub_details = is_string($item->std_and_sub_details) ? json_decode($item->std_and_sub_details, true) : $item->std_and_sub_details;
//         return $item;
//     });
...
// }
    ...
    public function store(Request $request)
    {
        ...
        $teacher = ClassTeacher::create([
            ...
            'std_and_sub_details' => json_encode($validatedData['std_and_sub_details']),
        ]);
    
        return response()->json(['message' => 'Teacher details stored successfully', 'data' => $teacher], 201);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $teacher->update([
            'class_teacher' => $validatedData['class_teacher'] ?? $teacher->class_teacher,
            'std_and_sub_details' => $validatedData['std_and_sub_details'] ?? $teacher->std_and_sub_details,
        ]);

        return response()->json(['message' => 'Teacher details updated successfully', 'data' => $teacher]);
    }

    // Delete teacher details
    // public function destroy($id)
    // {
    //     $teacher = ClassTeacher::where('teacher_id', $teacher_id)->first();
    ...
    public function destroy($id)
    {
        ...
        return response()->json(['message' => 'Teacher soft deleted successfully']);
    }
