...
class InvoiceController extends Controller
{
      ...
      public function getReceiptListshort(Request $request)
    {
        ...
        // Initialize query
         $query = ByPayInformation::select('transactionId', 'amount', 'created_at');
        ...
        if (isset($requestData['invoiceId'])) {
            ...
            $invoiceLists = DB::table('generate_invoice_views')
                ...
                ->leftJoin('student_infos', function ($join) {
                    $join->on('generate_invoice_views.student_id', '=', 'student_infos.id');
                })
                ->where('generate_invoice_views.invoice_no', '=', $invoiceNo)
                ->select('generate_invoice_views.*', 'generate_invoice_views.roll_no as student_roll_number', 'generate_invoice_views.sec as student_sec', 'generate_invoice_views.created_at as invoiceCreatedDate', 'invoice_lists.*', 'invoice_lists.created_at as paymentDate', 'student_infos.*')
            ...
            if ($invoiceLists) {
                ...
                $invoiceDetails->section = strtoupper($invoiceLists->student_sec);
                $invoiceDetails->rollNo = $invoiceLists->student_roll_number;
                $invoiceDetails->group = $invoiceLists->Group;
                $invoiceDetails->invoiceDate = date('d/m/Y', strtotime(str_replace('-', '/', $invoiceLists->invoiceCreatedDate)));
                $invoiceDetails->invoiceNo = $invoiceLists->invoice_no;
                $invoiceDetails->total = $invoiceLists->amount;
                $invoiceDetails->previousDues = $invoiceLists->previous_pending_amount;
                $invoiceDetails->discount = $invoiceLists->discount_percent ? $invoiceLists->discount_percent : NULL;
                $invoiceDetails->amount = $invoiceLists->amount;
                $invoiceDetails->invoice_pending_amount = $invoiceLists->invoice_pending_amount;
                $invoiceDetails->totalPayment = $invoiceLists->total_invoice_amount;
                $returnData->invoiceDetails = $invoiceDetails;
                $returnData->paymentDetails = json_decode($invoiceLists->fees_items_details);
                // $returnData->discount_list = json_decode($invoiceLists->discount_items_details);
                if ($invoiceLists->fees_cat == 'school') {
                    $returnData->discount_list = json_decode($invoiceLists->discount_items_details);
                }else  if ($invoiceLists->fees_cat !== 'school') { 
            ...
            $bypayDetails = DB::table('by_pay_informations')
             ...
             $latestbypayDetails = DB::table('by_pay_informations')
                                ...
                                ->latest('id')
                                ->first();
            return response()->json(["data" => $returnData, 'bypayDetails' => $bypayDetails, 'latestbypayDetails' => $latestbypayDetails, 'invoiceDetails' => $invoiceDetails])->header("Access-Control-Allow-Origin",  "*");
        } else {
 ...
 $paymentInformation = DB::table('by_pay_informations')->where('student_id', $invoice->student_id)->where('type', $invoice->fees_cat)->latest('id')->first();
    ...
    if (!in_array($invoice->slno, $slnoSet)) {
        $slnoSet[] = $invoice->slno;
                $data[] = [
                    ...
                    'dwonloadReceipt' => $invoice->payment_status == 'Paid' || $invoice->payment_status == 'Partial Paid'  ? true : false,
            ...
            return response()->json(['data' => $data, 'bypayDetails' => $bypayDetails, 'latestbypayDetails' => $latestbypayDetails, 'invoiceDetails' => $invoiceDetails], 200)->header("Access-Control-Allow-Origin",  "*");
        }
    }
    public function getsponinfo() {
        $sponserorstudentDetails = DB::table('by_pay_informations')
        ->select( 'id', 'student_id', 'invoice_id', 'transactionId', 'amount', 'sponsor', )
        ->selectRaw('CONVERT_TZ(created_at, "+00:00", "+05:30") as created_at')
        ->where('sponsor', '!=', '')
        ->get();
        ...
        foreach ($sponserorstudentDetails as $sponserorstudentDetail) {
            ...
            $user = User::where('id', $sponserorstudentDetail->sponsor)->first();
            $stu_user = User::where('id', $sponserorstudentDetail->student_id)->first();
        ...
        return response()->json(["sponserorstudentDetail" => $sponserorstudentDetails])->header("Access-Control-Allow-Origin", "*");
    }
       public function getPaymentReceiptList(Request $request)
    {
        ...
        if (isset($requestData['slno'])) {
            ...
            $paidReceiptList = DB::table('generate_invoice_views')
                ->leftjoin('invoice_lists', 'invoice_lists.invoice_id', 'generate_invoice_views.slno')
              //  ->where('invoice_lists.transaction_completed_status', 1)
                ->where('invoice_lists.status', 'success')
                ->where('generate_invoice_views.slno', $slNo)
                ->select('generate_invoice_views.*', 'invoice_lists.payment_transaction_id')
                ->get();
                foreach ($paidReceiptList as $sponsorOption) {
                    $sponsorOption->payment_transaction_id = (string)$sponsorOption->payment_transaction_id;
                }
    ...
    public function getPaymentReceipt(Request $request)
    {
        ...
        if (isset($requestData['transactionNo'])) {
          ...
          $sponserorstudentDetails  =DB::table('by_pay_informations')->where(
        ...
        $latestReceipt = ByPayInformation::where('student_id', $sponserorstudentDetails->student_id)
                                         ->whereNull('inv_amt') 
        ...
        $disable_delete= null;
        if ($latestReceipt && $latestReceipt->id == $sponserorstudentDetails->id) {
    ...
    ->first();
        //  return response()->json(["data" => $invoiceListsA]);
            $transactionDetails = DB::table('payment_orders_statuses')
                ->leftjoin('payment_orders_details', 'payment_orders_details.internal_txn_id', 'payment_orders_statuses.clnt_txn_ref')
                ->where('payment_orders_statuses.clnt_txn_ref', $transactionNo) ->orWhere('payment_orders_statuses.clnt_txn_ref', $invoiceListsA->unique_payment_transaction_id)
                ->select(
                    'payment_orders_statuses.*',
                    'payment_orders_details.*',
                    'payment_orders_statuses.created_at as paymentDateTime',
                    DB::raw("CAST(payment_orders_details.internal_txn_id AS CHAR) AS internalTxnId")
                )
                ->first(); //get all invoices
               $by_pay_info= ByPayInformation::where('transactionId', $transactionNo)->first();
             $Student_user = User::where('id', $by_pay_info->student_id)->first();
            $paidinvoiceDetails = DB::table('generate_invoice_views')
                ->leftjoin('invoice_lists', 'invoice_lists.invoice_id', 'generate_invoice_views.slno', 'generate_invoice_views.name', 'generate_invoice_views.standard', 'generate_invoice_views.roll_no', 'generate_invoice_views.sponser_id')
            ...
            $transactionDetailsData = (object)[];
             if ($transactionDetails) {
                $transactionDetailsData->transactionNo = $transactionDetails->internalTxnId;
                $transactionDetailsData->date = date('d/m/Y', strtotime($transactionDetails->paymentDateTime));
                $transactionDetailsData->time = date('h:i:sa', strtotime($transactionDetails->paymentDateTime));
                $transactionDetailsData->modeOfPayment = $transactionDetails->paymentMode;
                $transactionDetailsData->paymentStatus = $transactionDetails->txn_msg;
                $transactionDetailsData->paymentAmounttwo = number_format($transactionDetails->amount, 2, '.', '');
        ...
        $transactionDetailsData->Student = Student::where('roll_no', $Student_user->roll_no)->where('admission_no', $Student_user->admission_no)->first();
 ...
 //   return response()->json(["data" => $paidinvoiceDetails])->header("Access-Control-Allow-Origin",  "*");
            ...
            $transactionDetailsData->invoice = $paidinvoiceDetails ;
                ...
                $transactionDetailsData->bankTransactionID = $transactionDetails->BankTransactionID;
            }
            if ($transactionDetails) {
                $tuserdetails =  User::where('id', $transactionDetails->user_id)->first() ;
            ...
            if ($paidinvoiceDetails) {
                ...
                foreach ($paidinvoiceDetails as $paidinvoiceDetail) {
                    ...
                    $invoiceData->amount = number_format($paidinvoiceDetail->total_invoice_amount, 2, '.', '');
                    $invoiceData->pending_amount = number_format($paidinvoiceDetail->invoice_pending_amount, 2, '.', '');
                    ...
                    $invoiceData->Sponsername = User::where('id', $paidinvoiceDetail->sponser_id)->value('name') ?? '';
                    if (!is_null($paidinvoiceDetail->sponser_id)) {
                        $user = SponserMaster::where('user_id', $paidinvoiceDetail->sponser_id)->first();
                        ...
                        $invoiceData->address = '<b>' . $sponserName . '</b>,<br>' . $sponserCompanyName . ', ' . $sponserCity . ', ' . $sponserLocation . ', ' . $sponserPincode;

                        $invoiceData->gst = SponserMaster::where('user_id', $paidinvoiceDetail->sponser_id)->value('gst') ?? '';
                        $invoiceData->pan = SponserMaster::where('user_id', $paidinvoiceDetail->sponser_id)->value('pan') ?? '';
                    }
                    array_push($invoiceDetails, $invoiceData);
                }
            }
            $transactionDetailsData->transactionNo =$transactionDetailsData->ByPayInformation->transactionId ;
            ...
            $returnData->sponserorstudentDetails = $sponserorstudentDetails ?? '';
            $returnData->disable_delete = $disable_delete ?? '';
            $returnData->sponserDetailsponsor_info = $sponserDetailsponsor_info ?? '';
            $returnData->sponserDetailsponsor_info = $sponserDetailsponsor_info ?? '';

        }
        return response()->json(["data" => $returnData])->header("Access-Control-Allow-Origin",  "*");
    }
    public function getSponsorSelectOptions()
{
    $sponsorOptions = User::select('id', 'name', 'email', 'excess_amount', 'h_excess_amount')
    ...
    foreach ($sponsorOptions as $sponsorOption) {
        ...
        $schoolExcess = ($sponsorOption->excess_amount !== null && $sponsorOption->excess_amount != 0)
            ? 'S(Rs. ' . $sponsorOption->excess_amount . ')'
            : 'S(Rs. 0)';
        
        $hostelExcess = ($sponsorOption->h_excess_amount !== null && $sponsorOption->h_excess_amount != 0)
            ? 'H(Rs. ' . $sponsorOption->h_excess_amount . ')'
            : 'H(Rs. 0)';

        // Update the name field by appending both school and hostel excess amounts
        $sponsorOption->name = $sponsorOption->name . ' - ' . $schoolExcess . ' - ' . $hostelExcess;
    }
    ...
    public function getSponsorSelectOptionstwo()
    {
        ...
        foreach ($sponsorOptions as $sponsorOption) {
            $sponsorOption->sponsername = $sponsorOption->name;
            if ($sponsorOption->excess_amount !== null && $sponsorOption->excess_amount !== 0) {
                // Update the 'name' field by appending the excess_amount
                $sponsorOption->name = $sponsorOption->name . '-S(Rs. ' . $sponsorOption->excess_amount.')- H('. $sponsorOption->h_excess_amount.')';
            } else {
    ...
    public function getParentSelectOptions()
    {
        ...
        foreach ($sponsorOptions as $sponsorOption) {
            if ($sponsorOption->excess_amount !== null && $sponsorOption->excess_amount !== 0) {
                // Update the 'name' field by appending the excess_amount
                $sponsorOptiont[] = [
                    'name' => $sponsorOption->name . ' - Rs. ' . $sponsorOption->excess_amount,
                    'id' => $sponsorOption->id,
    ...
    public function getsponserstudent(Request $request)
    {
        ...
        foreach ($sponsorIds as $sponsor) {
            $studentRecords[$sponsor->id] = [
                ...
                'students' => User::select('id', 'admission_no', 'roll_no', 'name', 'gender', 'standard', 'twe_group', 'sec', 'hostelOrDay', 'email')
    ...
    public function getSponsorIDStudents($sponsorId)
    {
        ...
        $students = User::where('sponser_id', $sponsorId)
            ->where('status', 1)
            ->select('id', 'admission_no', 'roll_no', 'name', 'gender', 'standard', 'twe_group', 'sec', 'hostelOrDay', 'email')
            ->get();
        ...
        foreach ($students as $student) {
            $studentRecord = GenerateInvoiceView::where('student_id', $student->id)
                ...
                foreach ($studentRecord as $invoiceRecord) {
                    if (isset($invoiceRecord->slno)) {
                        ...
                        if (isset($invoiceRecord->invoice_pending_amount) && $invoiceRecord->invoice_pending_amount !== null && $invoiceRecord->invoice_pending_amount !== 0) {
                            $pendingAmount = $invoiceRecord->invoice_pending_amount;
                            // $studentRecord->pendingAmountwithconditon = $pendingAmount;
                        } else {
                            $pendingAmount = $invoiceRecord->total_invoice_amount;
                            // $studentRecord->pendingAmountwithconditon = $pendingAmount;
                        }
        ...
        if (empty($studentRecords)) {
            ...
            return response()->json(['message' => 'No records found for this sponsor.'], 404);
        }

        // Return a JSON response containing the students' records
        return response()->json(['studentsInvoice' => $studentRecords, 'studentinfo' => $studentinfo]);
    }
    ...
    public function getSponsorIDStudentstwo($sponsorId, Request $request)
    {
        ...
        foreach ($students as $student) {
            $studentRecord = GenerateInvoiceView::where('student_id', $student->id)->where('fees_cat', $sponsorType)->get();
            // Loop through the students and retrieve their records
            // Check if $studentRecord is not empty
            if (!$studentRecord->isEmpty()) {
                    ...
                    if (isset($invoiceRecord->slno)) {
                        ...
                        if (isset($invoiceRecord->payment_status)) {
                            if ($invoiceRecord->payment_status === 'Paid') {
                                $pendingAmount = '';
                            } elseif ($invoiceRecord->payment_status === 'Partial Paid') {
                                $pendingAmount = $invoiceRecord->invoice_pending_amount;
                            } else {
                                $pendingAmount = $invoiceRecord->total_invoice_amount;
                            }
                          ...
                          $paymentInformation = DB::table('by_pay_informations')->where('student_id', $invoiceRecord->student_id)->where('type', $invoiceRecord->fees_cat)->latest('id')->first();
        $most_recent_dues = $paymentInformation->due_amount;
...
//dd($users);
        ...
        if ($users->isEmpty()) {
            // Return a custom JSON response indicating no records found
            return response()->json(['message' => 'No records found with non-null excess_amount.'], 404);
        }
  ...
  public function updateExcessAmount(Request $request)
{
    ...
    if (empty($updateData)) {
        return response()->json(['message' => 'No valid excess amounts to update.'], 400);
    }
    ...
    return response()->json(['message' => 'Excess amounts updated successfully.']);
}
    ...
    public function viewpreExcessAmount($id)
    {
        ...
        // // Optionally, you can return a success message or updated user details
        // return response()->json([$user]);
        $classes = DB::table('user_excess_histories')->where('sponser_id', $userId)->get();
    ...
    // {
    //     $invoiceIds = $request->input('ids'); // An array of invoice IDs
    //     $totalAmount = $request->input('amount'); // Total payment amount

    //     // Fetch the invoice records based on the array of IDs
    //     $invoiceRecords = GenerateInvoiceView::whereIn('slno', $invoiceIds)->get();

    //     // Calculate the split amount for each invoice
    //     $totalInvoiceAmount = $invoiceRecords->sum('total_invoice_amount');
    //     $splitAmount = min($totalAmount, $totalInvoiceAmount);

    //     $remainingAmount = $totalAmount - $splitAmount; // Initialize remaining amount

    //     foreach ($invoiceRecords as $invoice) {
    //         if ($splitAmount > 0) {
    //             // Calculate the payment amount for this invoice
    //             $paymentAmount = min($splitAmount, $invoice->total_invoice_amount);

    //             // Update the invoice status and payment details
    //             $paymentStatus = ($paymentAmount < $invoice->total_invoice_amount) ? 'Partial Paid' : 'Paid';

    //             GenerateInvoiceView::where('invoice_no', $invoice->invoice_no)
    //                 ->update([
    //                     'payment_status' => $paymentStatus,
    //                     'invoice_status' => 4,
    //                     'paid_amount' => $paymentAmount,
    //                     'invoice_pending_amount' => $invoice->total_invoice_amount - $paymentAmount,
    //                     'additionalDetails' => $request->input('additionalDetails'),
    //                     'mode' => $request->input('mode')
    //                 ]);

    //             // Update the remaining split amount
    //             $splitAmount -= $paymentAmount;
    //         }
    //     }

    //     // Return a response indicating the success of the operation and the remaining amount
    //     return response()->json(['message' => 'Payment processed successfully', 'status' => 'Success', 'remaining_amount' => $remainingAmount]);
    // }
    ...
    //     // Calculate the total invoice count
    //     $totalInvoiceCount = $invoiceRecords->count();

    //     if ($totalInvoiceCount === 0) {
    //         return response()->json(['message' => 'No invoices selected.', 'status' => 'Error']);
    //     }

    //     // Calculate the split amount for each invoice
    //     $splitAmount = intval($totalAmount / $totalInvoiceCount);

    //     // Calculate the remaining amount after equal distribution
    //     $remainingAmount = $totalAmount % $totalInvoiceCount;

    //     foreach ($invoiceRecords as $invoice) {
    //         if ($splitAmount > 0) {
    //             // Update the invoice status and payment details
    //             $paymentStatus = ($splitAmount < $invoice->total_invoice_amount) ? 'Partial Paid' : 'Paid';
    ...
    //                     'invoice_pending_amount' => $invoice->total_invoice_amount - $splitAmount,
    //                     'additionalDetails' => $request->input('additionalDetails'),
    //                     'mode' => $request->input('mode')
    //                 ]);

    //             // Subtract the split amount from the total amount
    //             $totalAmount -= $splitAmount;
    //         }

    //         // If there's a remaining amount, distribute it evenly
    //         if ($remainingAmount > 0) {
    //             // Distribute the remaining amount among the invoices
    //             $extraAmount = 1; // Distribute 1 unit at a time
    //             $paymentStatus = ($extraAmount < $invoice->total_invoice_amount) ? 'Partial Paid' : 'Paid';

    //             GenerateInvoiceView::where('invoice_no', $invoice->invoice_no)
    //                 ->update([
    //                     'payment_status' => $paymentStatus,
    //                     'invoice_status' => 4,
    //                     'paid_amount' => $extraAmount,
    //                     'invoice_pending_amount' => $invoice->total_invoice_amount - $extraAmount,
    //                     'additionalDetails' => $request->input('additionalDetails'),
    //                     'mode' => $request->input('mode')
    //                 ]);

    //             // Subtract the extra amount from the remaining amount
    //             $remainingAmount -= $extraAmount;
    //         }
    //     }

    //     // Return a response indicating the success of the operation
    //     return response()->json(['message' => 'Payment processed successfully', 'status' => 'Success']);
    // }
    ...
    //     $invoiceData = $request->input('invoiceData'); // An array of invoice data
    //     $totalAmount = $request->input('totalAmount'); // Total payment amount
    //     return response()->json(['message' => $invoiceData]);
    //     // Calculate the sum of individual invoice amounts
    //     $totalInvoiceAmount = collect($invoiceData)->sum('amount');


    //     if ($sponsorId) {
    //         $sponsor = User::where('id', $sponsorId)->first();
    //         if ($sponsor) {
    //             // If the sponsor has excess_amount, add the extraAmount to it
    //             if ($sponsor->excess_amount === null || $sponsor->excess_amount == 0) {

    //                 if ($totalAmount == 0) {
    //                     return response()->json(['message' => 'No Excess or Total amount .', 'status' => 'amountFalse']);
    //                 } else {
    //                     if ($totalAmount > $totalInvoiceAmount) {
    //                         $extraAmount = $totalAmount - $totalInvoiceAmount;
    //                     } elseif ($totalAmount == $totalInvoiceAmount) {
    //                         $extraAmount = 0;
    //                      } else {
    //                         return response()->json(['message' => 'Invoice amount is greater than total given amount.', 'status' => 'amountFalse']);
    //                     }
    //                 }
    //             } else {
    //                 if ($totalAmount === 0 || $totalAmount === null) {
    //                     if ($totalInvoiceAmount <= $sponsor->excess_amount) {
    //                         $extraAmount = $totalInvoiceAmount - $sponsor->excess_amount;
    //                     } elseif ($totalInvoiceAmount > $sponsor->excess_amount) {
    //                         return response()->json(['message' => 'Invoice amount is greater than total given amount added with excess amount.', 'status' => 'amountFalse']);
    //                     }
    //                 } elseif (($totalAmount + $sponsor->excess_amount) > $totalInvoiceAmount) {
    //                     $excesss_amount = $sponsor->excess_amount;
    //                     $extraAmount = $totalAmount + $excesss_amount - $totalInvoiceAmount;
    //                 } else {
    //                     return response()->json(['message' => 'Invoice amount is greater than total given amount even added with excess amount.', 'status' => 'amountFalse']);
    //                 }
    //             }
    //             //   $sponsor->save(); // Save the updated sponsor's excess_amount
    //         }
    //     }
    //     // Check if there is any extra amount

    //     if ($extraAmount < 0) {
    //         return response()->json(['message' => 'Total amount is less than the sum of individual invoice amounts.', 'status' => 'amountFalse']);
    //     }

    //     // Initialize a flag to track successful updates
    //     $allUpdatesSuccessful = true;

    //     // Iterate through each invoice data and update the records
    //     foreach ($invoiceData as $data) {
    //         $id = $data['id'];
    //         $invoiceNo = $data['invoice_no'];
    //         $amount = $data['amount'];

    //         // Fetch the invoice record based on ID and invoice number
    //         $invoiceRecord = GenerateInvoiceView::where('slno', $id)->where('invoice_no', $invoiceNo)->first();

    //         if (!$invoiceRecord) {
    //             $allUpdatesSuccessful = false;
    //             continue; // Skip to the next invoice if not found
    //         }

    //         // Calculate the pending amount based on invoice_pending_amount if available
    //         // if (isset($invoiceRecord->invoice_pending_amount) && $invoiceRecord->invoice_pending_amount !== null && $invoiceRecord->invoice_pending_amount !== 0) {
    //         //     $pendingAmount = $invoiceRecord->invoice_pending_amount - $amount;
    //         // } else {
    //         // $pendingAmount = $invoiceRecord->total_invoice_amount - $amount;
    //         // }
    //         if ($invoiceRecord->total_invoice_amount < $amount) {
    //             $pendingAmount = $invoiceRecord->total_invoice_amount - $amount;
    //             DB::table('invoice_pendings')->insert([
    //                 'fees_cat' => $invoiceRecord->fees_cat,
    //                 'student_id' => $invoiceRecord->student_id,
    //                 'invoice_no' => $invoiceRecord->invoice_no,
    //                 'pending_amount' => $pendingAmount,
    //             ]);
    //         } elseif (($invoiceRecord->total_invoice_amount > $amount)) {
    //             $pendingAmount = $invoiceRecord->total_invoice_amount - $amount;
    //             DB::table('invoice_pendings')->insert([
    //                 'fees_cat' => $invoiceRecord->fees_cat,
    //                 'student_id' => $invoiceRecord->student_id,
    ...
    //         // Update the invoice status and payment details
    //         $paymentStatus = ($amount < $invoiceRecord->total_invoice_amount) ? 'Partial Paid' : 'Paid';

    //         $updateResult = GenerateInvoiceView::where('slno', $id)
    //             ->where('invoice_no', $invoiceNo)
    //             ->update([
    //                 'payment_status' => $paymentStatus,
    //                 'invoice_status' => 4,
    ...
    //         // If the update was not successful, set the flag to false
    //         if (!$updateResult) {
    //             $allUpdatesSuccessful = false;
    //         } else {
    //             //New update
    ...
    //             $payment_order_data['user_id'] = $invoiceRecord->student_id;  // user id need to mention 
    //             $payment_order_data['amount'] = $amount;
    //             $payment_order_data['maxAmount'] = null;
    //             //Payment user details
    //             $payment_order_data['name'] = $invoiceRecord->name;
    //             $payment_order_data['custID']  = $invoiceRecord->student_id;
    //             $payment_order_data['mobNo'] = null;

    //             //Payment 
    //             $payment_order_data['paymentMode'] = 'cash';
    ...
    //                 'user_uuid' => $invoiceRecord->student_id, // user id need to mention here
    //                 'invoice_id' => $invoiceRecord->slno,
    ...
    //         // Add the extra amount to the sponsor's excess_amount (if available)
    //         if ($sponsorId) {
    //             $sponsor = User::where('id', $sponsorId)->first();
    //             if ($sponsor) {
    //                 // If the sponsor has excess_amount, add the extraAmount to it
    //                 if ($sponsor->excess_amount === null) {
    //                     $sponsor->excess_amount = $extraAmount;
    //                 } else {
    //                     $sponsor->excess_amount = $extraAmount;
    //                 }
    //                 $sponsor->save(); // Save the updated sponsor's excess_amount
    ...
    //         // Log the request data to the 'bulk_sponser_payments' table
    ...
    //         return response()->json(['message' => 'Payment processed successfully', 'status' => 'Success']);
    //     } else {
    //         return response()->json(['message' => 'Payment processing failed for some invoices.', 'status' => 'Error']);
    //     }
    // }
...
public function processCashPayment(Request $request)
{
    ...
    if($sponsor_prev_excess < $totalAmount){
        return response()->json(['message' => 'Total amount is greater than sponsor excess amount.', 'status' => 'NoAmount']);
    }
    if($sponsortype == "school"){
    $sponsorremainexcess = $sponsor_prev_excess - $totalAmount;
    
    User::where('id', $sponsorId)->update(['excess_amount' => $sponsorremainexcess]);
    } else {
         $sponsorremainexcess = $sponsor_prev_excess - $totalAmount;
    
    User::where('id', $sponsorId)->update(['h_excess_amount' => $sponsorremainexcess]);
    }
    ...
    foreach ($invoiceData as $invoice) {
        ...
        if (!$invoiceDetails) {
            return response()->json(['message' => 'Invoice details not found.'], 404);
        }
        
            
        // Generate a random transaction ID for each invoice
        $transactionId =  FastInvoiceHelper::generateReceiptWithPrefix($invoiceDetails->fees_cat);
        ...
        $studentcheckexcess = User::where('id', $invoiceDetails->student_id)->first();
         if($invoiceDetails->fees_cat == "school"){
                ...
                User::where('id', $invoiceDetails->student_id)->update(['excess_amount' => $student_prev_excess + $school_excess]);
                ...
                User::where('id', $invoiceDetails->student_id)->update(['h_excess_amount' => $student_prev_excess + $hostel_excess]);
            }

        // Update the invoice record
        GenerateInvoiceView::where('invoice_no', $invoiceDetails->invoice_no)
            ->update([
                'payment_status' => $payment_status,
                'invoice_status' => 4,
                'paid_amount' => number_format(($invoiceDetails->paid_amount ?? 0) + $payed_amount, 2, '.', ''),
                'invoice_pending_amount' => number_format($pending_amount, 2, '.', ''),
 ...
 $dataInvoicePaymentMaps = [
    ...
    public function getNoDuesCertificatesbyid($id)
    {
        ...
        $studentIds = User::where('id', $id)->pluck('id')->toArray();
        ...
        foreach ($studentIds as $studentId) {
            ...
            // $studentId = 13;
            $student = $student = User::select('id', 'admission_no', 'roll_no', 'name', 'gender', 'standard', 'twe_group', 'sec', 'hostelOrDay', 'email', 'fee_by', 'sponser_id')
            ...
            if ($student) {
                ...
                $pendingInvoices = GenerateInvoiceView::where('student_id', $studentId)
                    ->where('payment_status', '<>', 'Paid') // Check for payment status other than 'Paid'
                    ->get();

                if ($pendingInvoices->isEmpty()) {
                    // No pending invoices found, the student has no dues
                    $issuedCertificates[] = ['student_info' => $student, 'acad_year' => $acad_year ?? '', 'message' => 'No dues found for the student. Issuing No Due Certificate...', 'status' => 'No Dues Certificate Can be Issued'];
                } else {
                    // There are pending invoices, the student has dues
                    $pendingDues[] = ['student_info' => $student, 'message' => 'Student has pending dues', 'status' => 'Pending Dues', 'pending_invoices' => $pendingInvoices];
                }
            } else {
                // Student not found
                $pendingDues[] = ['student_info' => $student, 'message' => 'Student not found', 'status' => 'Error'];
            }
        }
    ...
    public function getNoDuesCertificatesAndPendingDues(Request $request)
    {
        ...
        // Get a list of all student IDs from the 'users' table
        $studentIds = User::where('user_type', 'student')->pluck('id')->toArray();
    ...
    public function sendsms(Request $request)
    {
        ...
        if ($GenerateInvoiceView->isEmpty()) {
            ...
            return response()->json(['message' => 'No unpaid invoices found for the specified slno values.']);
        }
        $invoices = [];
...
$feesItemsDetails = json_decode($invoice->fees_items_details, true);
$discountItemsDetails = json_decode($invoice->discount_items_details, true);
$paymentInformation = DB::table('by_pay_informations')->where('student_id', $invoice->student_id)->where('type', $invoice->fees_cat)->latest('id')->first();
//fees_items_details discount_items_details   amount  total_invoice_amount discount_percent
                    ...
                    Log::info('Email for invoice ' . $invoice->invoice_no . ' queued for sending to ' . $userDatas->email);

                    // Your SMS sending code here (if applicable)

                    // Log a success message indicating that the SMS was sent successfully
                    Log::info('SMS sent successfully for invoice ' . $invoice->invoice_no);
                ...
                Log::error('Error processing notifications for invoice ' . $invoice->invoice_no . ': ' . $e->getMessage());
            }
        }

        return response()->json(['message' => 'SMS and email notifications sent successfully.', 'invoiceMail' => $invoices,'invoiceMfeeail' => json_decode($invoice->fees_items_details)]);
    }
}
