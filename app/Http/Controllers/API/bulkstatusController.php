            ...
            DB::transaction(function () use ($ids, $status, $todayDate) {
                foreach ($ids as $id) {
                    ...
                    if ($student) {
                        ...
                        Log::info("Student ID {$id} status updated to {$status} on {$todayDate}");
    
                        // Update AdmissionForm only if it exists
                        $admission = AdmissionForm::find($student->admission_id);
                        if ($admission) {
...
//   if (strtolower($request->status) === 'active') {
        ...
        if (!$student->roll_no) {
            ...
            $classOfJoining = strtolower($student->SOUGHT_STD);
            ...
            $student->update(['roll_no' => $newAdmissionNo]);
        }
    ...
    ->first();
                ...
                $admissionId =  $newAdmissionNo;
                            $student->update(['admission_no' => $admissionId]);

        }
 
         if (!User::where('roll_no', $student->roll_no)
                            ...
                            ->exists()) 
                    {
                      ...
                      try {
                        $userId =  User::where('roll_no', $student->roll_no)->latest('id')->value('id');
                        ...
                        \Log::error('Failed to log application active lifecycle.  Status Updated to Active', [
                            ...
                            Log::info("AdmissionForm ID {$admission->id} status updated to {$status} on {$todayDate}");
                        } else {
                            // Log if AdmissionForm is not found
                            Log::warning("AdmissionForm not found for Student ID {$id} on {$todayDate}");
                        }
                    } else {
                        // Log if Student is not found
                        Log::error("Student ID {$id} not found on {$todayDate}");
                    }
                }
            });
            ...
            // Log the error
            Log::error("Error updating status on {$todayDate}: " . $e->getMessage());
