...
class StaffController extends Controller
{
     public function addStaff(Request $request)
    {
        ...
        $requestData['permanentAddress'] = json_encode($requestData['permanentAddress']);
    }

    if (isset($requestData['communicationAddress'])) {
        $requestData['communicationAddress'] = json_encode($requestData['communicationAddress']);
    }
    $requestData['staff_id'] = Autogeneratenumber::generateStaffId($requestData['date_of_joining']);

        $staff = Staff::create($requestData);
    if ($request->has('staff_photo')) {
    // request, $staff, fieldname on API, file path, db column name
    $this->handleImageUpdate($request, $staff, 'staff_photo', 'staff_photo', 'staff_photo');
}
...
{ 
   ...
   $requestData = $request->validate([
    ...
    return response()->json(['message' => 'Staff details updated successfully!', 'staff' => $staff], 200);
}
  ...
  if ($staff->permanentAddress) {
        $staff->permanentAddress = json_decode($staff->permanentAddress, true);
    }
   if ($staff->staff_photo) {
        $staff->staff_photo = asset('storage/app/public/' . $staff->staff_photo);
    } else {
        $staff->staff_photo = null; // If no photo, set to null
    }
    if ($staff->communicationAddress) {
        $staff->communicationAddress = json_decode($staff->communicationAddress, true);
    }
...
public function viewAllStaff(Request $request)
{
    ...
    if ($request->has('from_date_joining') && $request->has('to_date_joining')) {
    ...
    if ($request->has('from_date_resignation') && $request->has('to_date_resignation')) {
        ...
        $fromDateResignation = date('Y-m-d', strtotime($fromDateResignation));
    ...
    $staff = $query->where('isdeleted', false)->orWhereNull('isdeleted')->get();

    // Decode JSON fields back to arrays
    foreach ($staff as $staffMember) {
        if ($staffMember->permanentAddress) {
            $staffMember->permanentAddress = json_decode($staffMember->permanentAddress, true);
        }

        if ($staffMember->communicationAddress) {
            $staffMember->communicationAddress = json_decode($staffMember->communicationAddress, true);
        }

        if ($staffMember->staff_photo) {
            $staffMember->staff_photo = asset('storage/app/public/' . $staffMember->staff_photo);
        } else {
    ...
    try {
        if ($request->has($fieldNameApi)) {
            ...
            if (preg_match('/^data:image\/(\w+);base64,/', $base64Image, $matches)) {
                ...
                $fullPath = Storage::disk('public')->path($filePathWithName);
