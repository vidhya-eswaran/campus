   ...
   $query = StudentAttendance::query();
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json(['message' => 'Attendance saved successfully.', 'data' => $attendance], 201);
        } catch (\Exception $e) {
            Log::error('Error storing attendance: ' . $e->getMessage());
            return response()->json(['message' => 'Failed to store attendance.'], 500);
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return response()->json(['message' => 'Failed to delete attendance.'], 500);
        }
    }
    ...
    //             if (!isset($item['roll_no']) || !isset($item['name'])) {
    //                 return response()->json(['message' => 'Each record must have roll_no and name.'], 422);
    //             }
    //         }
    ...
    //         return response()->json(['message' => 'Bulk records inserted successfully.'], 201);
    //     } catch (\Exception $e) {
    ...
    try {
        ...
        foreach ($data as $item) {
            ...
            if (
                ...
                Log::warning('Missing required fields, skipping record:', ['item' => $item['roll_no']]);
                continue;
            }
            ...
            try {
                ...
                Log::info('Successfully processed record:', ['match' => $match, 'update' => $update]);
            } catch (\Exception $e) {
        ...
        Log::error('Bulk upsert error: ' . $e->getMessage(), ['error' => $e]);
